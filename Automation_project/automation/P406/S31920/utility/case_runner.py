...
class Test_Info(object):
    ...
    @staticmethod    
    def copy(test_info_obj):
        new_obj = copy.deepcopy(test_info_obj)
        return new_obj
    ...
    def test_finished(self):
        ...
        self.start = self.test_result.start_time
        self.end = self.test_result.stop_time
...
class Test_Result(_TextTestResult):
    """
    ...
    def __init__(self, json_name = None, stream=sys.stderr, descriptions=1, verbosity=1,
                 elapsed_times=True):
        _TextTestResult.__init__(self, stream, descriptions, verbosity)
        self.successes = []
        self.callback = None
        self.elapsed_times = elapsed_times
        self.test_json_name = json_name
        self.target_list = []

    def _prepare_callback(self, test_info, target_list, verbose_str,
                          short_str):
        """
        Appends a Test_Info to the given target list and sets a callback
    ...
    def addSuccess(self, test):
        ...
        self._prepare_callback(
            Test_Info(self, test), self.successes, 'OK', '.'
        )
    ...
    def addFailure(self, test, err):
        ...
        """
        testinfo = Test_Info(self, test, Test_Info.ERROR, err)
        self.errors.append((
            testinfo,
    ...
    def addSkip(self, test, reason):
        ...
        """
        testinfo = Test_Info(self, test, Test_Info.SKIP, reason)
    ...
    def _report_testcase(self, suite_name, test_index, test_result, xml_testsuite, xml_document):
        """
        ...
        testcase.setAttribute(
            'name', Test_Result._test_method_name(test_result.test_id)
        )
        testcase.setAttribute('time', '%.3f' % test_result.elapsed_time)

        if (test_result.outcome != Test_Info.SUCCESS):
            elem_name = ('failure', 'error', 'skipped')[test_result.outcome - 1]
            failure = xml_document.createElement(elem_name)
            testcase.appendChild(failure)
            if test_result.outcome != Test_Info.SKIP:
    ...
    def _report_testsuite(run_start_time, run_end_time, test_json_name, suite_name, outsuffix, tests, xml_document):
        """
        ...
        if test_json_name:
            testsuite.setAttribute('json', test_json_name)
        testsuite.setAttribute('runner_start_time', run_start_time.strftime("%Y%m%d-%H%M%S.%f")[:-3])
        testsuite.setAttribute('runner_end_time', run_end_time.strftime("%Y%m%d-%H%M%S.%f")[:-3])        

        testsuite.setAttribute(
            'time', '%.3f' % sum(map(lambda e: e.elapsed_time, tests))
        )
        failures = filter(lambda e: e.outcome == Test_Info.FAILURE, tests)
        testsuite.setAttribute('failures', str(len(list(failures))))

        errors = filter(lambda e: e.outcome == Test_Info.ERROR, tests)
    ...
    def generate_reports(self, test_runner):
        ...
        for suite, tests in all_results.items():
            ...
            # Build the XML file
            testsuite = Test_Result._report_testsuite(
                test_runner.runner_start_time, test_runner.runner_end_time, self.test_json_name, suite, test_runner.outsuffix, tests, doc
            )

            for test_index in range(0, len(tests)):
                self._report_testcase(suite, test_index, tests[test_index], testsuite, doc)
            Test_Result._report_output(test_runner, testsuite, doc)
...
class Test_Runner(TextTestRunner):
    ...
    def __init__(self, json_name=None, output='.', outsuffix=None, stream=sys.stderr,
        ...
        if self.test_json_name:
            suff = json_name + "-" + time.strftime("%Y%m%d%H%M%S")
        else :
            suff = time.strftime("%Y%m%d%H%M%S")
            
        outsuffix = outsuffix is not None and outsuffix + suff or suff
    ...
    def _make_result(self):
        return Test_Result(
            self.test_json_name, self.stream, self.descriptions, self.verbosity, self.elapsed_times
        )
    ...
    def run(self, test):
        ...
        try:
            ...
            if self.test_json_name:
                self.stream.writeln('Json file name is ' + self.test_json_name)
            ...
            test(result)
            self.runner_end_time = datetime.datetime.now()
            time_taken = (self.runner_end_time - self.runner_start_time).total_seconds()
            ...
            if self.test_json_name:
                self.stream.writeln("Ran %d test%s in %.3fs against JSON file %s" % (
                    run, run != 1 and "s" or "", time_taken, self.test_json_name)
                )
            else: 
