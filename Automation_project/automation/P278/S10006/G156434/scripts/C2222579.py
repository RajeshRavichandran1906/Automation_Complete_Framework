'''
Created on 29-NOV-2016

@author: Aftab

Test Suite = http://lnxtestrail.ibi.com/testrail/index.php?/suites/view/10006
Test Case = http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/2222579
TestCase Name = Verify Right Justify applied
'''
import unittest
from common.lib import utillity
from common.lib import core_utility
from common.pages import visualization_metadata,visualization_resultarea, visualization_ribbon, ia_resultarea, ia_run, ia_styling
from common.lib.basetestcase import BaseTestCase
from selenium.webdriver.common.by import By

class C2222579_TestClass(BaseTestCase):

    def test_C2222579(self):
        
        Test_Case_ID = "C2222579"
        utillobj = utillity.UtillityMethods(self.driver)
        core_util_obj = core_utility.CoreUtillityMethods(self.driver)
        metaobj = visualization_metadata.Visualization_Metadata(self.driver)
        resultobj = visualization_resultarea.Visualization_Resultarea(self.driver)
        ribbonobj = visualization_ribbon.Visualization_Ribbon(self.driver)
        ia_resultobj = ia_resultarea.IA_Resultarea(self.driver)
        ia_runobj = ia_run.IA_Run(self.driver)
        ia_stylingobj = ia_styling.IA_Style(self.driver)
        selection_list = ["COUNTRY","CAR","DEALER_COST", "RETAIL_COST"]
        
        """ 1. Launch IA Report mode:
               http://machine:port/ibi_apps/ia?tool=Report&master=ibisamp/CAR&item=IBFS%3A%2FWFC%2FRepository%2FS10006    """
        utillobj.infoassist_api_login('report','ibisamp/car','P278/S10006', 'mrid', 'mrpass')
        elem1=(By.CSS_SELECTOR, "#TableChart_1")
        resultobj._validate_page(elem1)
        
        """ 2. Select "COUNTRY","CAR","DEALER_COST","RETAIL_COST".        """
        for text in selection_list:
            metaobj.datatree_field_click(text, 2, 1)
            utillobj.synchronize_with_visble_text("#queryTreeColumn", text, metaobj.chart_medium_timesleep)
        
        """ 3. Click Style in the Report section        """
        ribbonobj.select_ribbon_item('Home', 'style')
        
        """ 4. Click Right Justify               """
        """ 5. Click Apply                              """
        """ 6. Click OK                                 """
        ia_stylingobj.set_report_style(right_justify=True, btn_apply=True, btn_ok=True)
        elem1=(By.CSS_SELECTOR, "#TableChart_1")
        resultobj._validate_page(elem1)
        coln_list = ['COUNTRY', 'CAR', 'DEALER_COST', 'RETAIL_COST']
        resultobj.verify_report_titles_on_preview(4, 4, "TableChart_1", coln_list, "Step 6.1: Verify Canvas column titles After Apply Style")
        ia_resultobj.verify_report_data_set('TableChart_1', 10, 4, Test_Case_ID+"_Ds01.xlsx", 'Step 6.2: Verify report dataset After Apply Style')
        ia_resultobj.verify_report_cell_property("TableChart_1", 5, text_align='right', font_size='9pt', text_value='ENGLAND', msg='Step 6.3:')
        ia_resultobj.verify_report_cell_property("TableChart_1", 37, text_align='right', font_size='9pt', text_value='BMW', msg='Step 6.4:')
        
        
        """ 7. Click Run and verify output                """
        ribbonobj.select_top_toolbar_item('toolbar_run')
        utillobj.synchronize_with_number_of_element('[id^=ReportIframe-]', 1, metaobj.chart_medium_timesleep)
        core_util_obj.switch_to_frame()
        ia_runobj.verify_table_data_set("table[summary='Summary']", Test_Case_ID+"_run_Ds01.xlsx" , 'Step 7: Verify report dataset')
        ia_runobj.verify_table_cell_property("table[summary='Summary']", 1, 3, text_align='right', font_size='9pt', text_value='DEALER_COST', msg='Step 7.1:')
        ia_runobj.verify_table_cell_property("table[summary='Summary']", 6, 2, text_align='right', font_size='9pt', text_value='ALFA ROMEO', msg='Step 7.2:')
         
        """ 8. Click Save in the toolbar > Save As C2222579 > click Save            """
        core_util_obj.switch_to_default_content()
        ribbonobj.select_tool_menu_item('menu_save_as')
        utillobj.ibfs_save_as("se_"+Test_Case_ID)
         
        """ 9. Logout: 'http://machine:port/ibi_apps/service/wf_security_logout.jsp'        """
        utillobj.infoassist_api_logout()
         
        """ 10. Reopen saved FEX:
                http://machine:port/ibi_apps/ia?item=IBFS%3A%2FWFC%2FRepository%2FS10006%2FC2222579.fex&tool=Report       """
        uname = (By.CSS_SELECTOR,'input[id=SignonUserName]')
        resultobj._validate_page(uname)
        utillobj.login_wf('mrid', 'mrpass')
        utillobj.infoassist_api_edit("se_"+Test_Case_ID, 'report', 'S10006')
         
        """ 11. Verify Preview                """
        elem1=(By.CSS_SELECTOR, "#TableChart_1")
        resultobj._validate_page(elem1)
        coln_list = ['COUNTRY', 'CAR', 'DEALER_COST', 'RETAIL_COST']
        resultobj.verify_report_titles_on_preview(4, 4, "TableChart_1", coln_list, "Step 11.1: Verify Preview column titles")
        ia_resultobj.verify_report_data_set('TableChart_1', 10, 4, Test_Case_ID+"_Ds01.xlsx", 'Step 11.2: Verify report dataset')
        ia_resultobj.verify_report_cell_property("TableChart_1", 5, text_align='right', font_size='9pt', text_value='ENGLAND', msg='Step 11.3:')
        ia_resultobj.verify_report_cell_property("TableChart_1", 37, text_align='right', font_size='9pt', text_value='BMW', msg='Step 11.4:')

if __name__ == '__main__':
    unittest.main()