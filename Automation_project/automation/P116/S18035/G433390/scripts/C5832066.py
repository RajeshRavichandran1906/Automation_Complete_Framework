...
@author: Varun
Testcase ID : http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/5832066
Testcase Name : Verify 3D bars are displaying with surface walls.
'''
import unittest
...
class C5832066_TestClass(BaseTestCase):

    def test_C5832066(self):
        """
        ...
        Step 2: Execute attached 3D_Bars.fex using following URL
        http://machine:port/{alias}/run.bip?BIP_REQUEST_TYPE=BIP_RUN&BIP_folder=IBFS%253A%252FWFC%252FRepository%252FS18035&BIP_item=3D_Bars.fex
        """
        active_chart_obj.run_fex_using_api_url(folder_path, '3D_Bars', mrid='mrid', mrpass='mrpass')
        ...
        active_chart_obj.verify_number_of_risers_in_run_window('path', 6, 20, parent_css=run_window_css, msg='Step 3.4: Verify the number of risers')
        blue_risers = util_obj.validate_and_get_webdriver_objects(blue_riser_css, 'blue-riser')
        actual_color = Color.from_string(util_obj.get_element_css_propery(blue_risers[4], 'fill')).rgba
        expected_color = util_obj.color_picker('bar_blue', 'rgba')
        util_obj.asequal(actual_color, expected_color, 'Step 3.5: Verify the blue coloured riser')
        green_risers = util_obj.validate_and_get_webdriver_objects(green_riser_css, 'green-riser')
        actual_color = Color.from_string(util_obj.get_element_css_propery(green_risers[4], 'fill')).rgba
        expected_color = util_obj.color_picker('pale_green', 'rgba')
        util_obj.asequal( actual_color, expected_color, 'Step 3.6: Verify the green coloured riser')
        active_chart_obj.verify_legends_in_run_window(z_axis_labels, parent_css=run_window_css, msg='Step 3.7: Verify the legends')
        wall_elements = util_obj.validate_and_get_webdriver_objects(wall_css, "walls")
        util_obj.verify_visible_elements(wall_elements, 10, 'Step 3.8: Verify the walls visible')
