'''
Created on Nov 8, 2017

@author: Magesh

Test Suite = http://lnxtestrail.ibi.com/testrail//index.php?/suites/view/10670&group_by=cases:section_id&group_id=168200&group_order=asc
Test Case = http://lnxtestrail.ibi.com/testrail//index.php?/cases/view/2313701
TestCase Name = AHTML: Vertical Dual-Axis Stacked Line Chart Limit functionality.
'''

import unittest
import time
from common.lib.basetestcase import BaseTestCase
from common.pages import visualization_ribbon,visualization_resultarea,active_miscelaneous, ia_resultarea, visualization_metadata
from common.lib import utillity
from selenium.webdriver.common.by import By

class C2313701_TestClass(BaseTestCase):

    def test_C2313701(self):
        
        """
        TESTCASE VARIABLES
        """ 
            
        driver = self.driver #Driver reference object created
#         driver.implicitly_wait(15) #Intializing common implicit wait for throughout the test
        utillobj = utillity.UtillityMethods(self.driver)
        ribbonobj = visualization_ribbon.Visualization_Ribbon(self.driver)
        resobj=visualization_resultarea.Visualization_Resultarea(self.driver)
        miscelaneousobj = active_miscelaneous.Active_Miscelaneous(driver)
        ia_resultobj=ia_resultarea.IA_Resultarea(self.driver)
        metaobj = visualization_metadata.Visualization_Metadata(self.driver)
        
        """
        Step 01: Open FEX: http://machine:port/ibi_apps/ia?item=IBFS%3A%2FWFC%2FRepository%2FS10670%2FVertical_Stacked_Line.fex&tool=Chart
        """
        utillobj.infoassist_api_edit("Vertical_Stacked_Line", 'chart', 'P116/S10670', mrid='mrid', mrpass='mrpass')
#         utillobj.infoassist_api_edit("C2313701", 'chart', 'P116/S10670', mrid='mrid', mrpass='mrpass')
        parent_css="#pfjTableChart_1 g.chartPanel"
        resobj.wait_for_property(parent_css, 1)
        time.sleep(8)
 
        """
        Expect to see the following Preview pane, including axis on both sides of the canvas.
        """
        parent_css="#TableChart_1 svg g text[class*='mgroupLabel']"
        resobj.wait_for_property(parent_css, 10)
        parent_css= "#TableChart_1 svg g text[class*='yaxis-labels']"
        resobj.wait_for_property(parent_css, 8)
        time.sleep(3)
        resobj.verify_xaxis_title("TableChart_1", "CAR", "Step 01.1: Verify X-Axis Title")  
        expected_xval_list=['ALFA ROMEO', 'AUDI', 'BMW', 'DATSUN', 'JAGUAR', 'JENSEN', 'MASERATI', 'PEUGEOT', 'TOYOTA', 'TRIUMPH']
        expected_yval1_list=['0', '10K', '20K', '30K', '40K', '50K', '60K', '70K']
        resobj.verify_riser_chart_XY_labels("TableChart_1", expected_xval_list, expected_yval1_list, "Step 01.2: Verify XY labels")
        expected_yval2_list=['0', '400', '800', '1,200', '1,600']
        resobj.verify_riser_chart_XY_labels("TableChart_1", expected_xval_list, expected_yval2_list, "Step 01.3: Verify XY labels", y_custom_css="svg > g text[class^='y2axis-labels']")
        ia_resultobj.verify_number_of_chart_segment('TableChart_1', 4, 'Step 01.4: Verify Number of riser', custom_css="path[class^='riser']")
        time.sleep(1)
        utillobj.verify_chart_color("TableChart_1", "riser!s0!g0!mline!", "bar_blue1", "Step 01.5: Verify  1st bar color",attribute_type='stroke')
        legend=['DEALER_COST', 'WEIGHT', 'LENGTH', 'HEIGHT']
        resobj.verify_riser_legends("TableChart_1", legend, "Step 01.6: Verify Y-Axis legend")
            
        """
        Step 02: Click the Run button.
        """
        time.sleep(8)
        ribbonobj.select_top_toolbar_item('toolbar_run')
        time.sleep(5)
        utillobj.switch_to_frame(pause=2)
            
        """Expect to see the following Vertical Dual-Axis Stacked Line Chart."""
        time.sleep(3)
        parent_css="#MAINTABLE_wbody0 g.chartPanel"
        resobj.wait_for_property(parent_css, 1)
        parent_css="#MAINTABLE_wbody0 svg g text[class*='mgroupLabel']"
        resobj.wait_for_property(parent_css, 10)
        parent_css= "#MAINTABLE_wbody0 svg g text[class*='yaxis-labels']"
        resobj.wait_for_property(parent_css, 8)
        time.sleep(3)
        resobj.verify_xaxis_title("MAINTABLE_wbody0", "CAR", "Step 02.1: Verify X-Axis Title")  
        expected_xval_list=['ALFA ROMEO', 'AUDI', 'BMW', 'DATSUN', 'JAGUAR', 'JENSEN', 'MASERATI', 'PEUGEOT', 'TOYOTA', 'TRIUMPH']
        expected_yval1_list=['0', '10K', '20K', '30K', '40K', '50K', '60K', '70K']
        resobj.verify_riser_chart_XY_labels("MAINTABLE_wbody0", expected_xval_list, expected_yval1_list, "Step 02.2: Verify XY labels")
        expected_yval2_list=['0', '400', '800', '1,200', '1,600']
        resobj.verify_riser_chart_XY_labels("MAINTABLE_wbody0", expected_xval_list, expected_yval2_list, "Step 02.3: Verify XY labels", y_custom_css="svg > g text[class^='y2axis-labels']")
        ia_resultobj.verify_number_of_chart_segment('MAINTABLE_wbody0', 4, 'Step 02.4: Verify Number of riser', custom_css="path[class^='riser']")
        time.sleep(3)
        expected_tooltip_list=['CAR:  ALFA ROMEO', 'DEALER_COST:  16,235', 'Filter Chart', 'Exclude from Chart']
        ia_resultobj.verify_marker_tooltip('MAINTABLE_wbody0', 'marker!s0!g0!mmarker!', expected_tooltip_list, 'Step 02.5 : Verify tooltip')
        utillobj.verify_chart_color("MAINTABLE_wbody0", "riser!s0!g0!mline!", "bar_blue1", "Step 02.6: Verify  1st bar color",attribute_type='stroke')
        legend=['DEALER_COST', 'WEIGHT', 'LENGTH', 'HEIGHT']
        resobj.verify_riser_legends("MAINTABLE_wbody0", legend, "Step 02.7: Verify Y-Axis legend")
        miscelaneousobj.verify_chart_title('MAINTABLE_wbody0_ft', 'DEALER_COST, WEIGHT, LENGTH, HEIGHT by CAR', 'Step 02.8: Verify Chart Title')
        miscelaneousobj.verify_arChartToolbar('MAINTABLE_wmenu0', ['More Options','Advanced Chart','Original Chart'],"Step 02.9: Verify Chart toolbar")
        miscelaneousobj.verify_arChartToolbar('MAINTABLE_wmenu0', ['Aggregation'],"Step 02.10: Verify Chart toolbar", custom_css='.arChartMenuBarContainer .tabPagingText1 td[title]')
        miscelaneousobj.verify_arChartToolbar('MAINTABLE_wmenu0', ['Sum'],"Step 02.11: Verify Chart toolbar", custom_css=".arChartMenuBarContainer [id*='SUM']", text=True)
        time.sleep(1)
        utillobj.switch_to_default_content(pause=3)
        time.sleep(3)
            
        """
        Step 03: Double click on fields COUNTRY & MODEL to add them to the Horizontal axis bucket under CAR.
        """
        time.sleep(4)
        metaobj.datatree_field_click('COUNTRY', 2, 1)
        time.sleep(4)
        metaobj.datatree_field_click('MODEL', 2, 1)
           
        """
        Expect to see the following Preview pane, with Car, Country & Model under the Horizontal axis bucket.
        Also expect to see the additional sort fields along the X-axis under the chart.
        """
        time.sleep(3)
        parent_css="#TableChart_1 svg g text[class*='mgroupLabel']"
        resobj.wait_for_property(parent_css, 18)
        metaobj.verify_query_pane_field('Horizontal Axis', 'CAR', 1, "Step03a: Verify query pane Car under the Horizontal axis bucket")
        metaobj.verify_query_pane_field('Horizontal Axis', 'COUNTRY', 2, "Step03a: Verify query pane Country under the Horizontal axis bucket")
        metaobj.verify_query_pane_field('Horizontal Axis', 'MODEL', 3, "Step03a: Verify query pane Model under the Horizontal axis bucket")
        resobj.verify_xaxis_title("TableChart_1", "CAR : COUNTRY : MODEL", "Step 03.1: Verify X-Axis Title")  
        expected_xval_list=['ALFA ROMEO : ITALY : 2000 4 DOOR...', 'ALFA ROMEO : ITALY : 2000 GT VEL...', 'ALFA ROMEO : ITALY : 2000 SPIDER...', 'AUDI : W GERMANY : 100 LS 2 DOO...', 'BMW : W GERMANY : 2002 2 DOOR', 'BMW : W GERMANY : 2002 2 DOOR...', 'BMW : W GERMANY : 3.0 SI 4 DOOR', 'BMW : W GERMANY : 3.0 SI 4 DOOR...', 'BMW : W GERMANY : 530I 4 DOOR', 'BMW : W GERMANY : 530I 4 DOOR...', 'DATSUN : JAPAN : B210 2 DOOR AUTO', 'JAGUAR : ENGLAND : V12XKE AUTO', 'JAGUAR : ENGLAND : XJ12L AUTO', 'JENSEN : ENGLAND : INTERCEPTOR...', 'MASERATI : ITALY : DORA 2 DOOR', 'PEUGEOT : FRANCE : 504 4 DOOR', 'TOYOTA : JAPAN : COROLLA 4 DOOR...', 'TRIUMPH : ENGLAND : TR7']
        expected_yval1_list=['0', '5K', '10K', '15K', '20K', '25K', '30K', '35K']
        resobj.verify_riser_chart_XY_labels("TableChart_1", expected_xval_list, expected_yval1_list, "Step 03.2: Verify XY labels")
        expected_yval2_list=['0', '40', '80', '120', '160', '200', '240', '280']
        resobj.verify_riser_chart_XY_labels("TableChart_1", expected_xval_list, expected_yval2_list, "Step 03.3: Verify XY labels", y_custom_css="svg > g text[class^='y2axis-labels']")
        ia_resultobj.verify_number_of_chart_segment('TableChart_1', 4, 'Step 03.4: Verify Number of riser', custom_css="path[class^='riser']")
        time.sleep(1)
        utillobj.verify_chart_color("TableChart_1", "riser!s0!g0!mline!", "bar_blue1", "Step 03.5: Verify  1st bar color",attribute_type='stroke')
        legend=['DEALER_COST', 'WEIGHT', 'LENGTH', 'HEIGHT']
        resobj.verify_riser_legends("TableChart_1", legend, "Step 03.6: Verify Y-Axis legend")
           
        """
        Step 04: Double click field BODYTYPE.
        """
        time.sleep(4)
        metaobj.datatree_field_click('BODYTYPE', 2, 1)
        time.sleep(4)
           
        """
        Expect to see the following Preview pane, with Car, Country & BODYTYPE under the Horizontal axis bucket.
        BODYTYPE replaced Model.
        Horizontal axis(sort) is limited to 3 fields.
        """
        parent_css="#TableChart_1 svg g text[class*='mgroupLabel']"
        resobj.wait_for_property(parent_css, 13)
        metaobj.verify_query_pane_field('Horizontal Axis', 'CAR', 1, "Step04a: Verify query pane Car under the Horizontal axis bucket")
        metaobj.verify_query_pane_field('Horizontal Axis', 'COUNTRY', 2, "Step04b: Verify query pane Country under the Horizontal axis bucket")
        metaobj.verify_query_pane_field('Horizontal Axis', 'BODYTYPE', 3, "Step04c: Verify query pane BODYTYPE under the Horizontal axis bucket")
        resobj.verify_xaxis_title("TableChart_1", "CAR : COUNTRY : BODYTYPE", "Step 04.1: Verify X-Axis Title")  
        expected_xval_list=['ALFA ROMEO : ITALY : COUPE', 'ALFA ROMEO : ITALY : ROADSTER', 'ALFA ROMEO : ITALY : SEDAN', 'AUDI : W GERMANY : SEDAN', 'BMW : W GERMANY : SEDAN', 'DATSUN : JAPAN : SEDAN', 'JAGUAR : ENGLAND : CONVERTIBLE', 'JAGUAR : ENGLAND : SEDAN', 'JENSEN : ENGLAND : SEDAN', 'MASERATI : ITALY : COUPE', 'PEUGEOT : FRANCE : SEDAN', 'TOYOTA : JAPAN : SEDAN', 'TRIUMPH : ENGLAND : HARDTOP']
        expected_yval1_list=['0', '10K', '20K', '30K', '40K', '50K', '60K', '70K']
        resobj.verify_riser_chart_XY_labels("TableChart_1", expected_xval_list, expected_yval1_list, "Step 04.2: Verify XY labels")
        expected_yval2_list=['0', '400', '800', '1,200', '1,600']
        resobj.verify_riser_chart_XY_labels("TableChart_1", expected_xval_list, expected_yval2_list, "Step 04.3: Verify XY labels", y_custom_css="svg > g text[class^='y2axis-labels']")
        ia_resultobj.verify_number_of_chart_segment('TableChart_1', 4, 'Step 04.4: Verify Number of riser', custom_css="path[class^='riser']")
        time.sleep(1)
        utillobj.verify_chart_color("TableChart_1", "riser!s0!g0!mline!", "bar_blue1", "Step 04.5: Verify  1st bar color",attribute_type='stroke')
        legend=['DEALER_COST', 'WEIGHT', 'LENGTH', 'HEIGHT']
        resobj.verify_riser_legends("TableChart_1", legend, "Step 04.6: Verify Y-Axis legend")
           
        """
        Step 05: Click the Run button. Hove over the point on the lower line for the first Alfa Romeo line..
        """
        time.sleep(8)
        ribbonobj.select_top_toolbar_item('toolbar_run')
        time.sleep(5)
        utillobj.switch_to_frame(pause=2)
            
        """Expect to see that Country and Bodytype follow Car in the Tooltip, as sort fields(Horizontal axis bucket)."""
        time.sleep(3)
        parent_css="#MAINTABLE_wbody0 g.chartPanel"
        resobj.wait_for_property(parent_css, 1)
        parent_css="#MAINTABLE_wbody0 svg g text[class*='mgroupLabel']"
        resobj.wait_for_property(parent_css, 13)
        parent_css= "#MAINTABLE_wbody0 svg g text[class*='yaxis-labels']"
        resobj.wait_for_property(parent_css, 8)
        time.sleep(3)
        resobj.verify_xaxis_title("MAINTABLE_wbody0", "CAR : COUNTRY : BODYTYPE", "Step 05.1: Verify X-Axis Title")  
        expected_xval_list=['ALFA ROMEO : ITALY : COUPE', 'ALFA ROMEO : ITALY : ROADSTER', 'ALFA ROMEO : ITALY : SEDAN', 'AUDI : W GERMANY : SEDAN', 'BMW : W GERMANY : SEDAN', 'DATSUN : JAPAN : SEDAN', 'JAGUAR : ENGLAND : CONVERTIB...', 'JAGUAR : ENGLAND : SEDAN', 'JENSEN : ENGLAND : SEDAN', 'MASERATI : ITALY : COUPE', 'PEUGEOT : FRANCE : SEDAN', 'TOYOTA : JAPAN : SEDAN', 'TRIUMPH : ENGLAND : HARDTOP']
        expected_yval1_list=['0', '10K', '20K', '30K', '40K', '50K', '60K', '70K']
        resobj.verify_riser_chart_XY_labels("MAINTABLE_wbody0", expected_xval_list, expected_yval1_list, "Step 05.2: Verify XY labels")
        expected_yval2_list=['0', '400', '800', '1,200', '1,600']
        resobj.verify_riser_chart_XY_labels("MAINTABLE_wbody0", expected_xval_list, expected_yval2_list, "Step 05.3: Verify XY labels", y_custom_css="svg > g text[class^='y2axis-labels']")
        ia_resultobj.verify_number_of_chart_segment('MAINTABLE_wbody0', 4, 'Step 05.4: Verify Number of riser', custom_css="path[class^='riser']")
        time.sleep(2)
        bar=['CAR:  BMW', 'COUNTRY:  W GERMANY', 'BODYTYPE:  SEDAN', 'HEIGHT:  337', 'Filter Chart', 'Exclude from Chart']
        ia_resultobj.verify_marker_tooltip('MAINTABLE_wbody0', 'marker!s3!g4!mmarker!', bar, 'Step 05.5 : Verify tooltip')
        utillobj.verify_chart_color("MAINTABLE_wbody0", "riser!s0!g0!mline!", "bar_blue1", "Step 05.6: Verify  1st bar color",attribute_type='stroke')
        legend=['DEALER_COST', 'WEIGHT', 'LENGTH', 'HEIGHT']
        resobj.verify_riser_legends("MAINTABLE_wbody0", legend, "Step 05.7: Verify Y-Axis legend")
        miscelaneousobj.verify_chart_title('MAINTABLE_wbody0_ft', 'DEALER_COST, WEIGHT, LENGTH, HEIGHT by CAR, COUNTRY, BODYTYPE', 'Step 05.8: Verify Chart Title')
        miscelaneousobj.verify_arChartToolbar('MAINTABLE_wmenu0', ['More Options','Advanced Chart','Original Chart'],"Step 05.9: Verify Chart toolbar")
        miscelaneousobj.verify_arChartToolbar('MAINTABLE_wmenu0', ['Aggregation'],"Step 05.10: Verify Chart toolbar", custom_css='.arChartMenuBarContainer .tabPagingText1 td[title]')
        miscelaneousobj.verify_arChartToolbar('MAINTABLE_wmenu0', ['Sum'],"Step 05.11: Verify Chart toolbar", custom_css=".arChartMenuBarContainer [id*='SUM']", text=True)
        time.sleep(1)
        utillobj.switch_to_default_content(pause=3)
        time.sleep(3)
           
        """
        Step 06: Delete fields Car & Bodytype from the Horizontal Axis bucket.
        Add fields RETAIL_COST, SALES & RPM to the Vertical Axis 1 bucket.
        Add fields WIDTH, WHEELBASE, FUEL_CAP, BHP, MPG & ACCEL to the Vertical Axis 2 bucket.
        """
        time.sleep(2)
        metaobj.querytree_field_click("CAR", 1, 1)
        time.sleep(1)
        utillobj.select_or_verify_bipop_menu('Delete')
        time.sleep(4)
           
        metaobj.querytree_field_click("BODYTYPE", 1, 1)
        time.sleep(1)
        utillobj.select_or_verify_bipop_menu('Delete')
        time.sleep(8)
           
        metaobj.datatree_field_click('RETAIL_COST', 2, 1)
        time.sleep(4)
        parent_css="#TableChart_1 svg g text[class*='mgroupLabel']"
        resobj.wait_for_property(parent_css, 5)
           
        metaobj.datatree_field_click('SALES', 2, 1)
        time.sleep(4)
        parent_css="#TableChart_1 path[class^='riser']"
        resobj.wait_for_property(parent_css, 6)
           
        metaobj.datatree_field_click('RPM', 2, 1)
        time.sleep(4)
        parent_css="#TableChart_1 path[class^='riser']"
        resobj.wait_for_property(parent_css, 7)
        
        time.sleep(10)
        metaobj.drag_drop_data_tree_items_to_query_tree('WIDTH', 1, 'HEIGHT', 0)
        parent_css='#queryTreeWindow tr:nth-child(15) td'
        resobj.wait_for_property(parent_css, 1,string_value='WIDTH',expire_time=20)
        time.sleep(4)
         
        metaobj.drag_drop_data_tree_items_to_query_tree('WHEELBASE', 1, 'WIDTH', 0)
        parent_css='#queryTreeWindow tr:nth-child(16) td'
        resobj.wait_for_property(parent_css, 1,string_value='WHEELBASE',expire_time=20)
        time.sleep(5)
         
        metaobj.drag_drop_data_tree_items_to_query_tree('FUEL_CAP', 1, 'WHEELBASE', 0)
        parent_css='#queryTreeWindow tr:nth-child(17) td'
        resobj.wait_for_property(parent_css, 1,string_value='FUEL_CAP',expire_time=20)
        time.sleep(5)
        
        metaobj.drag_drop_data_tree_items_to_query_tree('BHP', 1, 'FUEL_CAP', 0)
        parent_css='#queryTreeWindow tr:nth-child(18) td'
        resobj.wait_for_property(parent_css, 1,string_value='BHP',expire_time=20)
        time.sleep(4)
        
        metaobj.drag_drop_data_tree_items_to_query_tree('MPG', 1, 'BHP', 0)
        parent_css='#queryTreeWindow tr:nth-child(19) td'
        resobj.wait_for_property(parent_css, 1,string_value='MPG',expire_time=20)
        time.sleep(4)
        
        metaobj.drag_drop_data_tree_items_to_query_tree('ACCEL', 1, 'MPG', 0)
        parent_css='#queryTreeWindow tr:nth-child(20) td'
        resobj.wait_for_property(parent_css, 1,string_value='ACCEL',expire_time=20)
        time.sleep(8)
         
        """
        Expect to see the Preview pane with 5 fields in the Vertical Axis 1 bucket and 8 fields in the Vertical Axis 2 bucket.
        """
        parent_css= "#TableChart_1 svg g text[class*='yaxis-labels']"
        resobj.wait_for_property(parent_css, 7)
        time.sleep(10)
        metaobj.verify_query_pane_field('Vertical Axis 1', 'DEALER_COST', 1, "Step06a: Verify query pane DEALER_COST under the Vertical Axis 1 bucket")
        metaobj.verify_query_pane_field('Vertical Axis 1', 'WEIGHT', 2, "Step06b: Verify query pane WEIGHT under the Vertical Axis 1 bucket")
        metaobj.verify_query_pane_field('Vertical Axis 1', 'RETAIL_COST', 3, "Step06c: Verify query pane RETAIL_COST under the Vertical Axis 1 bucket")
        metaobj.verify_query_pane_field('Vertical Axis 1', 'SALES', 4, "Step06d: Verify query pane SALES under the Vertical Axis 1 bucket")
        metaobj.verify_query_pane_field('Vertical Axis 1', 'RPM', 5, "Step06e: Verify query pane RPM under the Vertical Axis 1 bucket")
        time.sleep(1)
        metaobj.verify_query_pane_field('Vertical Axis 2', 'LENGTH', 1, "Step06f: Verify query pane LENGTH under the Vertical Axis 2 bucket")
        metaobj.verify_query_pane_field('Vertical Axis 2', 'HEIGHT', 2, "Step06g: Verify query pane HEIGHT under the Vertical Axis 2 bucket")
        metaobj.verify_query_pane_field('Vertical Axis 2', 'WIDTH', 3, "Step06h: Verify query pane WIDTH under the Vertical Axis 2 bucket")
        metaobj.verify_query_pane_field('Vertical Axis 2', 'WHEELBASE', 4, "Step06i: Verify query pane WHEELBASE under the Vertical Axis 2 bucket")
        metaobj.verify_query_pane_field('Vertical Axis 2', 'FUEL_CAP', 5, "Step06j: Verify query pane FUEL_CAP under the Vertical Axis 2 bucket")
        metaobj.verify_query_pane_field('Vertical Axis 2', 'BHP', 6, "Step06k: Verify query pane BHP under the Vertical Axis 2 bucket")
        metaobj.verify_query_pane_field('Vertical Axis 2', 'MPG', 7, "Step06l: Verify query pane MPG under the Vertical Axis 2 bucket")
        metaobj.verify_query_pane_field('Vertical Axis 2', 'ACCEL', 8, "Step06m: Verify query pane ACCEL under the Vertical Axis 2 bucket")
        
        resobj.verify_xaxis_title("TableChart_1", "COUNTRY", "Step 06.1: Verify X-Axis Title")  
        expected_xval_list=['ENGLAND', 'FRANCE', 'ITALY', 'JAPAN', 'W GERMANY']
        expected_yval1_list=['0', '40K', '80K', '120K', '160K', '200K', '240K']
        resobj.verify_riser_chart_XY_labels("TableChart_1", expected_xval_list, expected_yval1_list, "Step 06.2: Verify XY labels")
        expected_yval2_list=['0', '500', '1,000', '1,500', '2,000', '2,500', '3,000', '3,500']
        resobj.verify_riser_chart_XY_labels("TableChart_1", expected_xval_list, expected_yval2_list, "Step 06.3: Verify XY labels", y_custom_css="svg > g text[class^='y2axis-labels']")
        ia_resultobj.verify_number_of_chart_segment('TableChart_1', 13, 'Step 06.4: Verify Number of riser', custom_css="path[class^='riser']")
        time.sleep(1)
        utillobj.verify_chart_color("TableChart_1", "riser!s0!g0!mline!", "bar_blue1", "Step 06.5: Verify  1st bar color",attribute_type='stroke')
        legend=['DEALER_COST', 'WEIGHT', 'RETAIL_COST', 'SALES', 'RPM', 'LENGTH', 'HEIGHT', 'WIDTH', 'WHEELBASE', 'FUEL_CAP', 'BHP', 'MPG', 'ACCEL']
        resobj.verify_riser_legends("TableChart_1", legend, "Step 06.6: Verify Y-Axis legend")
        
        """
        Step 07: Click the Run button.
        """
        time.sleep(8)
        ribbonobj.select_top_toolbar_item('toolbar_run')
        time.sleep(5)
        utillobj.switch_to_frame(pause=2)
         
        """Expect to see the following Line chart with 13 lines for each country."""
        time.sleep(3)
        parent_css="#MAINTABLE_wbody0 g.chartPanel"
        resobj.wait_for_property(parent_css, 1)
        parent_css="#MAINTABLE_wbody0 svg g text[class*='mgroupLabel']"
        resobj.wait_for_property(parent_css, 5)
        parent_css= "#MAINTABLE_wbody0 svg g text[class*='yaxis-labels']"
        resobj.wait_for_property(parent_css, 7)
        time.sleep(3)
        resobj.verify_xaxis_title("MAINTABLE_wbody0", "COUNTRY", "Step 07.1: Verify X-Axis Title")  
        expected_xval_list=['ENGLAND', 'FRANCE', 'ITALY', 'JAPAN', 'W GERMANY']
        expected_yval1_list=['0', '40K', '80K', '120K', '160K', '200K', '240K']
        resobj.verify_riser_chart_XY_labels("MAINTABLE_wbody0", expected_xval_list, expected_yval1_list, "Step 07.2: Verify XY labels")
        expected_yval2_list=['0', '500', '1,000', '1,500', '2,000', '2,500', '3,000', '3,500']
        resobj.verify_riser_chart_XY_labels("MAINTABLE_wbody0", expected_xval_list, expected_yval2_list, "Step 07.3: Verify XY labels", y_custom_css="svg > g text[class^='y2axis-labels']")
        ia_resultobj.verify_number_of_chart_segment('MAINTABLE_wbody0', 13, 'Step 07.4: Verify Number of riser', custom_css="path[class^='riser']")
        utillobj.verify_chart_color("MAINTABLE_wbody0", "riser!s0!g0!mline!", "bar_blue1", "Step 07.6: Verify  1st bar color",attribute_type='stroke')
        legend=['DEALER_COST', 'WEIGHT', 'RETAIL_COST', 'SALES', 'RPM', 'LENGTH', 'HEIGHT', 'WIDTH', 'WHEELBASE', 'FUEL_CAP', 'BHP', 'MPG', 'ACCEL']
        resobj.verify_riser_legends("MAINTABLE_wbody0", legend, "Step 07.7: Verify Y-Axis legend")
        miscelaneousobj.verify_chart_title('MAINTABLE_wbody0_ft', 'DEALER_COST, WEIGHT, RETAIL_COST, SALES, RPM, LENGTH, HEIGHT, WIDTH, WHEELBASE, FUEL_CAP, BHP, MPG, ACCEL by COUNTRY', 'Step 07.8: Verify Chart Title')
        miscelaneousobj.verify_arChartToolbar('MAINTABLE_wmenu0', ['More Options','Advanced Chart','Original Chart'],"Step 07.9: Verify Chart toolbar")
        miscelaneousobj.verify_arChartToolbar('MAINTABLE_wmenu0', ['Aggregation'],"Step 07.10: Verify Chart toolbar", custom_css='.arChartMenuBarContainer .tabPagingText1 td[title]')
        miscelaneousobj.verify_arChartToolbar('MAINTABLE_wmenu0', ['Sum'],"Step 07.11: Verify Chart toolbar", custom_css=".arChartMenuBarContainer [id*='SUM']", text=True)
        time.sleep(1)
        
        """
        Step 08: Hover over the point on the lowest line and the point on the top line for England.
        Expect to see the Dealer_Cost value relate to the left-side axis scale.
        Expect to see the Accel value relate to the right-side axis scale.
        """
        time.sleep(5)
        bar=['COUNTRY:  ENGLAND', 'DEALER_COST:  37,853', 'Filter Chart', 'Exclude from Chart']
        ia_resultobj.verify_marker_tooltip('MAINTABLE_wbody0', 'marker!s0!g0!mmarker!', bar, 'Step 08.1: Verify line value"')
        time.sleep(6) 
        parent_elem=driver.find_element_by_css_selector("#MAINTABLE_wbody0")
        utillobj.click_on_screen(parent_elem, 'start') 
        time.sleep(5)
        parent_elem=driver.find_element_by_css_selector("#MAINTABLE_wbody0 [class*='marker!s12!g0!mmarker!']")
        utillobj.click_on_screen(parent_elem, 'middle',javascript_marker_enable=True,mouse_duration=2.5)
        time.sleep(1)
#         bar=['COUNTRY:ENGLAND', 'ACCEL:22', 'Filter Chart', 'Exclude from Chart']
        bar=['COUNTRY:ENGLAND', 'ACCEL:23', 'Filter Chart', 'Exclude from Chart']
        resobj.verify_default_tooltip_values("MAINTABLE_wbody0", "marker!s12!g0!mmarker!",bar, msg="Step 08.2: Verify line value",default_move=True)
         
        
        """
        Step 09: Hover over the point on the bottom line and the point on the third line from the bottom for W GERMANY.
        Expect to see the Dealer_Cost value relate to the left-side axis scale.
        Expect to see the Length value relate to the right-side axis scale.
        """
        time.sleep(5)
        bar=['COUNTRY:  W GERMANY', 'DEALER_COST:  54,563', 'Filter Chart', 'Exclude from Chart']
        ia_resultobj.verify_marker_tooltip('MAINTABLE_wbody0', 'marker!s0!g4!mmarker!', bar, 'Step 09.1: Verify line value"')
        time.sleep(5) 
        bar=['COUNTRY:  W GERMANY', 'LENGTH:  1,309', 'Filter Chart', 'Exclude from Chart']
        ia_resultobj.verify_marker_tooltip('MAINTABLE_wbody0', 'marker!s5!g4!mmarker!', bar, 'Step 09.2: Verify line value"')
        time.sleep(8)
        element= driver.find_element_by_css_selector("#MAINTABLE_wbody0")
        utillobj.take_screenshot(element,'C2313701_Actual_step09', image_type='actual',x=1, y=1, w=-1, h=-1)
        
        """
        Step 10: Close the chart.
        Logout: http://machine:port/ibi_apps/service/wf_security_logout.jsp
        """
        time.sleep(1)
        utillobj.switch_to_default_content(pause=1)
        time.sleep(3)
        elem1=(By.CSS_SELECTOR, "#applicationButton img")
        resobj._validate_page(elem1)
        time.sleep(3)
        
if __name__ == '__main__':
    unittest.main()