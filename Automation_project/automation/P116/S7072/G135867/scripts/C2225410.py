...
Test Case = http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/2225410
...
class C2225410_TestClass(BaseTestCase):

    def test_C2225410(self):
        ...
        if browser == 'firefox':
            ...
            del action
        time.sleep(5)
        miscelanousobj.verify_highlighted_rows('ITableData0', 6, 'Step 02.02: Verify all the highlighted rows/values are now highlighted')
        
        
        """
        Step 3. Click Unhighlight All option on column value.
            
        """
        
        miscelanousobj.select_field_menu_items('ITableData0', 8, 1,'Unhighlight All')
        element=driver.find_element_by_css_selector('table[id="IWindowBody0"] .arGridBar')
        if browser == 'firefox':
            utillobj.click_type_using_pyautogui(element)
        else:
            ...
            del action
        time.sleep(5)
        miscelanousobj.verify_highlighted_rows('ITableData0', 0, 'Step 03.01 : Verify all the highlighted rows/values are now Unhighlighted')
