...
Test Case = http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/2251603
'''
import unittest
import time
...
class C2251603_TestClass(BaseTestCase):

    def test_C2251603(self):
        
        """ TESTCASE VARIABLES """
        Test_Case_ID = 'C2251603'
        driver = self.driver
        ...
        """    2. Now, select Check box from 'Insert' tab    """
        ribbonobj.select_ribbon_item("Insert", "Checkbox")
        ribbonobj.repositioning_document_component('8', '1')
        
        """    3. Right click on Checkbox prompt    """
        oListbox=driver.find_element_by_id("Prompt_1")
        utillobj.default_click(oListbox, click_option=1)
        utillobj.synchronize_with_visble_text("div[id^='BiPopup'][style*='inherit'] table tr[class*='bi-menu-item']:nth-child(5)", 'Properties', expire_time=4)
        ...
        #utillobj.create_data_set('ITableData0', 'I0r', 'C2251603_Ds01.xlsx')
        utillobj.verify_data_set('ITableData0', 'I0r', 'C2251603_Ds01.xlsx', 'Step 06c: Verify data.')
        p1=driver.find_elements_by_css_selector("#checkboxPROMPT_1 table tr")
        actual_prompt1_values=[el.text.strip() for el in p1]
        utillobj.asequal(actual_prompt1_values, oWith_ALL_expected, "Step 06d: Check 'All' option is present in the Check box")
        oCheck=driver.find_element_by_css_selector("#checkboxPROMPT_1 table tr input[checked]").get_attribute('value')
        utillobj.asequal(oCheck, '[All]', "Step 06e: Verify the first check box is 'All' and checked")
        ...
        #utillobj.create_data_set('ITableData0', 'I0r', 'C2251603_Ds02.xlsx')
        utillobj.verify_data_set('ITableData0', 'I0r', 'C2251603_Ds02.xlsx', 'Step 10c: Verify data.')
        p1=driver.find_elements_by_css_selector("#checkboxPROMPT_1 table tr")
        actual_prompt1_values=[el.text.strip() for el in p1]
        utillobj.asequal(actual_prompt1_values, oWithout_ALL_expected, "Step 10d: Check 'All' option is not present in the Check box")
        oCheck=driver.find_element_by_css_selector("#checkboxPROMPT_1 table tr input[checked]").get_attribute('value')
        utillobj.asequal(oCheck, '186534', "Step 10e: Verify the first check box is '186534' and checked")
        utillobj.switch_to_default_content()
        time.sleep(3)
