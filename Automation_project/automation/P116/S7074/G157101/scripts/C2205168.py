...
Test Case = http://lnxtestrail.ibi.com/testrail//index.php?/cases/view/2205168
TestCase Name = Verify a chart shows correct filtered output once filter applied from context menu - Scatter Chart
'''
import unittest
import time
...
class C2205168_TestClass(BaseTestCase):

    def test_C2205168(self):
        
        Test_Case_ID="C2205168"
        """
            TESTCASE VARIABLES
        """
        ...
        resobj.verify_yaxis_title("MAINTABLE_wbody0", yaxis_value, "Step 04.a(i): Verify -yAxis Title")
        time.sleep(3)
        xaxis_value ="Product"
        resobj.verify_xaxis_title("MAINTABLE_wbody0", xaxis_value, "Step 04.a(ii): Verify -xAxis Title")
        time.sleep(2)
        ...
        """
            Step 05:Hover over the point for Capuccino in the COFFEE Category.
            Expect to see values:
            Category: Coffee
            Product: Capuccino
            Unit Sales: 189,217
            Filter Chart
        ...
        expected_tooltip_list=['Category:Coffee', 'Product:Capuccino', 'Unit Sales:189217', 'Filter Chart', 'Exclude from Chart']
        resobj.verify_default_tooltip_values("MAINTABLE_wbody0", "riser!s0!g0!mmarker!r0!c0!", expected_tooltip_list, "Step 05: Verify bar value")
        parent_element_obj=self.driver.find_element_by_id('MAINTABLE_wbody0')
        utillobj.click_on_screen(parent_element_obj, 'top_middle')
        time.sleep(5)
        """
            Step 06:Click the Filter Chart option from the hover over menu..
        """
        resobj.select_default_tooltip_menu("MAINTABLE_wbody0","riser!s0!g0!mmarker!r0!c0!", 'Filter Chart')
        time.sleep(5)
        resobj.verify_yaxis_title("MAINTABLE_wbody0", "Unit Sales", "Step 05.2: Verify -yAxis Title")
        resobj.verify_xaxis_title("MAINTABLE_wbody0", "Product", "Step 05.3: Verify -xAxis Title")
        time.sleep(2)
        expected_xval_list=['Capuccino']
        expected_yval_list=['0', '40K', '80K', '120K', '160K', '200K']
        resobj.verify_riser_chart_XY_labels("MAINTABLE_wbody0", expected_xval_list, expected_yval_list, "Step 06.b: Verify XY labels")
        resobj.verify_visualization_row_column_header_labels('MAINTABLE_wbody0', 'columns', 'Category', ['Coffee'], "Step 06.c: Verify row header and value")
        utillobj.verify_chart_color('MAINTABLE_wbody0', 'riser!s0!g0!mmarker!r0!c0!', 'bar_blue', 'Step 06.d: Verify Color',attribute_type='stroke')
        ia_resultobj.verify_number_of_chart_segment('MAINTABLE_wbody0', 1, "Step 06.e: Verify number of Scatter", custom_css=".chartPanel .scrollCharts g circle[class*='marker']")
        ...
        expected_tooltip_list=['Category:Coffee', 'Product:Capuccino', 'Unit Sales:189217', 'Remove Filter']
        resobj.verify_default_tooltip_values("MAINTABLE_wbody0", "riser!s0!g0!mmarker!r0!c0!", expected_tooltip_list, "Step 06.g: Verify bar value")
        utillobj.switch_to_default_content(pause=3)
        time.sleep(2)
        ele=driver.find_element_by_css_selector("#resultArea")
        utillobj.take_screenshot(ele,Test_Case_ID + '_Actual_step 6', image_type='actual',x=1, y=1, w=-1, h=-1)
        time.sleep(2)
