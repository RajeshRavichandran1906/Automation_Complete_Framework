...
Test Case = http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/2204882
TestCase Name = Verify user is able to export chart to Excel document
...
class C2204882_TestClass(BaseTestCase):

    def test_C2204882(self):
        ...
        utillobj.take_monitor_screenshot('C2204882_Actual_Step_01','actual', left=0, top=60, right=0, bottom=40)
        time.sleep(4)
        
        """
        Step 02 : Click Export to > Excel option from the dropdown menu
        """
        browser=utillobj.parseinitfile('browser')
        if browser=='IE' :
            act_chart.select_chartmenubar_option('MAINTABLE_wmenu0',0,'Export to->Excel',0,custom_css='cpop')
            time.sleep(8)
            utillobj.take_monitor_screenshot('C2204882_Actual_Step_02','actual', left=0, top=0, right=0, bottom=40)
            utillobj.saveas_excel_sheet('C2204882_Actual_IE_.xlsx')
            time.sleep(4)
            utillobj.verify_excel_sheet('C2204882_Actual_IE_.xlsx', 'C2204882_Base_IE_.xlsx', 'Sheet1', 'Step 10: Expect to see the Excel spreadsheet')
        else :
