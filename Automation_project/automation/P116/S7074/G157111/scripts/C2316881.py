...
Test Case = http://lnxtestrail.ibi.com/testrail//index.php?/cases/view/2316881
TestCase Name = Verify that Gradient down fill is applied in the output.
'''
import unittest
import time
...
class C2316881_TestClass(BaseTestCase):

    def test_C2316881(self):
        
        Test_Case_ID="C2316881"
        """
            TESTCASE VARIABLES
        """   
        ...
        style_obj=ia_styling.IA_Style(driver)
        """
        ...
        parent_css="#pfjTableChart_1 g.chartPanel rect"
        utillobj.synchronize_with_number_of_element(parent_css, 25, ribbonobj.home_page_long_timesleep)
        ...
        resobj.verify_xaxis_title("TableChart_1", "Category", "Step 02.01: Verify -xAxis Title")
        time.sleep(2)
        expected_xval_list=['Coffee']
        expected_yval_list=['0', '100K', '200K', '300K', '400K', '500K','600K']
        resobj.verify_riser_chart_XY_labels("TableChart_1", expected_xval_list, expected_yval_list, "Step 02.02: Verify XY labels")
        resobj.verify_number_of_riser("TableChart_1", 1, 1, 'Step 02.03: Verify the total number of risers displayed on preview')
        time.sleep(1)
        ...
        utillobj.synchronize_with_visble_text('#frameOptionsVBox', 'Gradient fill', ribbonobj.home_page_long_timesleep)
        
        """   
            Step 04. Select Gradient fill, then select 'Gradient down' for 'Direction'..
        """
        ribbon_obj.select_frame_background_options('Frame', gradient_fill=True, direction='Gradient down', btnOk=True)
        time.sleep(5) 
        ...
        time.sleep(8)
        resobj.verify_xaxis_title("MAINTABLE_wbody0", "Category", "Step 05.01: Verify -xAxis Title")
        resobj.verify_yaxis_title("MAINTABLE_wbody0", "Unit Sales", "Step 05.02: Verify -yAxis Title")
        time.sleep(2)
        expected_xval_list=['Coffee','Food','Gifts']
        expected_yval_list=['0', '0.4M', '0.8M', '1.2M', '1.6M']
        resobj.verify_riser_chart_XY_labels("MAINTABLE_wbody0", expected_xval_list, expected_yval_list, "Step 05.03: Verify XY labels")
        resobj.verify_number_of_riser("MAINTABLE_wbody0", 1, 3, 'Step 05.04: Verify the total number of risers displayed on preview')
        time.sleep(1)
        utillobj.verify_chart_color("MAINTABLE_wbody0", "riser!s0!g0!mbar!", "bar_blue", "Step 05.05: Verify  bar color")
        miscelaneous_obj.verify_chart_title('MAINTABLE_wbody0_ft', 'Unit Sales by Category', 'Step 05.06: Verify Chart Title')
        miscelaneous_obj.verify_arChartToolbar('MAINTABLE_wmenu0', ['More Options','Advanced Chart','Original Chart'],"Step 05.07: Verify Chart toolbar")
        miscelaneous_obj.verify_arChartToolbar('MAINTABLE_wmenu0', ['Aggregation'],"Step 05.08: Verify Chart toolbar", custom_css='.arChartMenuBarContainer .tabPagingText1 td[title]')
        ...
        style_obj.verify_Gradient_fill_bgcolor('MAINTABLE_wbody0', 'white', 'Trolley_Grey', 'Step 05.11')
