...
class C2316897_TestClass(BaseTestCase):

    def test_C2316897(self):
        """
        TESTCASE VARIABLES
        """
        Test_Case_ID = 'C2316897'
        driver = self.driver
        ...
        ia_styleobj=ia_styling.IA_Style(self.driver)
        ...
        """    4. Select Background. Check the Show Border Color box. Change the color to Blue and click OK.    """        
        ribbon_obj.select_frame_background_options('Background',show_border_color=True, border_color='blue', btnOk=True)
        time.sleep(5)
        ...
        oLine=driver.find_element_by_css_selector("#MAINTABLE_0 rect.background").value_of_css_property('stroke') 
        expected_line_color=utillobj.color_picker('blue', 'rgb')
        utillobj.asequal(expected_line_color, oLine , "Step 05h: Expect to see the following Active Report with Blue Border.")
