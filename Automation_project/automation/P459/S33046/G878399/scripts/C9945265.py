...
class C9945265_TestClass(BaseTestCase):
    
    def test_C9945265(self):
        ...
        item_attib = [{'_jt' : 'IBFSGitRevCommit', 'fullMessage' : "'Commit Validation'"}]
        parent_item_attib = [{'_jt' : 'IBFSGitRevCommit', 'fullMessage' : 'Initial commit'}]
        
        STEP_01 = """
            STEP 01 : Sign into WebFOCUS and Run gitCommit with [IBFS_gitCommitOptions = --message 'Commit Validation', gitCommitParams = IBFS:/WFC/Repository/P459_S33046_Val_API]
        """
        self.Log.Message(STEP_01)
        self.signOn('devuser1', 'devpass1')
        git_commit = gitCommit(IBIRS_gitCommitOptions="--message 'Commit Validation'", IBIRS_gitCommitParams=IBIRS_path)
         
        STEP_01_EXPECTED = """
            STEP 01 Expected : Verify returncode="10000" and <item _jt="IBFSGitRevCommit" commitTime='' fullMessage='Commit Validation' /> and
            <item _jt="IBFSGitRevCommit" commitTime="" fullMessage="Initial commit"/>
        """
        ...
        git_commit.XMLAssertions.assertAttributesListIn(git_commit.XmlNode.IBFSGitRevCommit_parent_item, parent_item_attib, '01.03', match_case=True)
