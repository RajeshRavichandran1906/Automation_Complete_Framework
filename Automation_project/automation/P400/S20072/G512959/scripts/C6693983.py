...
Test Case = http://lnxtestrail.ibi.com/testrail//index.php?/cases/view/6693983
TestCase Name = Date YYMD format to DMtrYY format
'''
...
class C6693983_TestClass(BaseTestCase):

    def test_C6693983(self):
        ...
        """ Step 3: Under Date format select 31 Dec 2018
        """
        date_formobj.select_date_format('31 Dec 2018')
        
        """ Step 4: Click on OK
        """
        data_formobj.close_data_format_dialog_using_ok_button()
        data_formobj.verify_output_format_value('DMtYY', '3')
        
        """ Step 5: Insert DMtYY into the Format to parse (input) and click Show data format
        """
        data_formobj.set_text_to_format_to_parse_input_box('DMtYY')
        data_formobj.click_show_data_format_button()
        data_formobj.verify_data_format_dialog_is_visible('5')
        data_formobj.verify_selected_datatype('date', '5.1')
        date_formobj.verify_date_format('31 Dec 2018', '5.2')
        time.sleep(3)
