...
Test Case = http://lnxtestrail.ibi.com/testrail//index.php?/cases/view/6693985
TestCase Name = Date YYMD format to M format
'''
...
class C6693985_TestClass(BaseTestCase):

    def test_C6693985(self):
        ...
        """ Step 3: Click on drop down for Date format select 12 (month) 
        """
        date_formobj.select_date_format('12 (month)')
        
        """ Step 4: Click OK 
        """    
        data_formobj.close_data_format_dialog_using_ok_button()
        data_formobj.verify_output_format_value('M', '3')
        
        """ Step 5: Insert M into the Format to parse (input) and click Show data format
        """
        data_formobj.set_text_to_format_to_parse_input_box('M')
        data_formobj.click_show_data_format_button()
        data_formobj.verify_data_format_dialog_is_visible('5')
        data_formobj.verify_selected_datatype('date', '5.1')
        date_formobj.verify_date_format('12 (month)', '5.2')
        time.sleep(3)
