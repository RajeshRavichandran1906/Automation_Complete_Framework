...
Test Case = http://172.19.2.180/testrail/index.php?/cases/view/6667794
TestCase Name = Integer Format to Smaller Length Alpha Format
'''
...
class C6667794_TestClass(BaseTestCase):

    def test_C6667794(self):
        ...
        """ Step 3: Click on the Data type Abc tab, change the Length from 100 to 7, click OK
        """
        data_formobj.select_datatype('alpha')
        alp_formobj.modify_length_value_using_keybord_input_inside_dataformat_dialog('7')
        data_formobj.close_data_format_dialog_using_ok_button()
        data_formobj.verify_output_format_value('A7', '3')
        
        """ Step 4: Insert A7 into the Format to parse (input) and click Show data format
        """
        data_formobj.set_text_to_format_to_parse_input_box('A7')
        data_formobj.click_show_data_format_button()
        data_formobj.verify_data_format_dialog_is_visible('4.1')
        data_formobj.verify_selected_datatype('alpha', '4.2')
        alp_formobj.verify_length_inside_dataformat_dialog('7', '4.3')
        alp_formobj.verify_variable_length_checkbox_inside_dataformat_dialog('Variable length', False, '4.4')
        data_formobj.close_data_format_dialog_using_ok_button()
        time.sleep(3)
