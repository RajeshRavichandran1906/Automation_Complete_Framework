...
Test Case = http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/2349631
TestCase Name = Verify properties of a Blog
'''
...
class C2349631_TestClass(BaseTestCase):

    def test_C2349631(self):
        """
        TESTCASE VARIABLES
        """
        ...
        name_box_css=".properties-general-folder [data-ibx-type*='ibxText'] input"
        name_box_disable_css=".properties-general-folder [data-ibx-type*='ibxText'].ibx-widget-disabled input"
        radio_button_css=".properties-advanced-pane-tab [data-ibx-type='ibxRadioButtonSimple']"
        
        """ Step 1: Login as a WebFOCUS Developer
        """
        ...
        """ Step 3: Right click on P242_S10674_G171304/Blogs/Blog1 and select properties from the menu.
        ...
        """ Step 4: Explore General tab.
                    Verify View All, Title, Summary, Publish and Show are enabled and Tool is blog. Owner appears as "-" and save button is disabled .
        ...
        wf_mainobj.verify_property_dialog_value('Tool', 'text', "Step 4.7: Tool is blog", property_value='blog')
        ...
        try:
            name_box_elem=name_row_obj.find_element_by_css_selector(name_box_css)
            name_textbox_value=utillobj.get_attribute_value(name_box_elem, 'text_value')
            temp_status=name_textbox_value['text_value']
        except NoSuchElementException:
            temp_status=False
        try:
            name_row_obj.find_element_by_css_selector(name_box_disable_css)
            temp_status_=False
        except NoSuchElementException:
            temp_status_=True
        if temp_status_==True and temp_status==proj_sub_folder_item:
            status=temp_status
        else:
            status=False
        utillobj.asequal(proj_sub_folder_item, status, "Step 4.11: Verify Name is enabled by clicking on Edit button next to Name box.")
        
        """ Step 5: Click on Advanced tab.
                    Verify Thumbnail (Default, Embedded, Link radio buttons), Tags, Sort Order, Default Width, Default Height are enabled. Load in iframe is disabled.
        """
        ...
        wf_mainobj.verify_property_dialog_enable_disable('Load in iframe', 'radiobutton_value', 'No', "Step 5.5: Verify Load in iframe - is disabled.", tab_name='Advanced')
