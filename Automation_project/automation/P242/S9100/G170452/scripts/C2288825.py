...
Test Case = http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/2288825'''
...
class C2288825_TestClass(AS_BaseTestCase):
    def test_C2288825(self):
        ...
        select_display_command_lines=['down','down','down','down','down','right','down','down']
        select_message_viewer_off=['down','down','down','down','down','right']
        browser_classname="IEFrame"
        add_value_italy="ITALY"
        ...
        verify_display_command_lines=["SET GRAPHENGINE=GRAPH53","SET HTMLEMBEDIMG=ON"]
        ...
        as_utilobj.select_application_menu_options(send_keys=select_display_command_lines)
        time.sleep(2)
        ...
        as_utilobj.click_element_using_ui(edit_element=True,name=filter_control,write=add_value_italy)
        time.sleep(2)
        
        as_utilobj.click_element_using_ui(hyperlink_control=click_run)
        time.sleep(6)
        
        as_utilobj.Verify_Browser_Content(browser_classname,verify_msg_2,item_list=verify_display_command_lines,browser_close=True)
        time.sleep(3) 
