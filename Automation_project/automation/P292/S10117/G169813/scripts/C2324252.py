...
from common.pages import visualization_resultarea, wf_legacymainpage, vfour_portal_ribbon, vfour_portal_properties, vfour_portal_canvas, vfour_portal_run
...
#         
        ...
        """Step 17:Navigate URL to http://environment name:port/alias/legacyhome"""
        
        utillobj.navigate_to_legacyhomepage()
        parent_css = "#PortalResourcevBOX table tr td"
        resultobj.wait_for_property(parent_css, 1, expire_time=50, string_value=workspace, with_regular_exprestion=True)
        
        """ Step 18: Re run the portal to make sure the public panel is still there.
        """
        wf_mainpageobj.select_repository_menu(BIP_Portal_Path+'->BIP_xxx_Portal123_V4', 'Run')
        parent_css = "#BIPortalPanel [class*='bip-canvas'] [class*='bip-page']:not([style*='hidden'])"
        resultobj.wait_for_property(parent_css, 1, expire_time=25)
        ...
        """ Step 19: Right click on the page tab
        ...
        """ Step 20: Press F8 and Try to drag a folder onto the page
                    Verify that you now can add something onto the page
        """
        portal_ribbon.invoke_and_verify_wf_resource_tree(launch_point='keybord')
        time.sleep(3)
        item_path=BIP_Portal_Path+"->Bluehills"
