...
Test Case Link  =  http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/6157188
Test Case Title =  Adding optional other date range control to the Page
-----------------------------------------------------------------------------------------------"""

from common.lib.basetestcase import BaseTestCase
from common.wftools import page_designer

class C6157188_TestClass(BaseTestCase):

    def test_C6157188(self):
        ...
        CONTAINER_ITEM='18 - Date Range Optional Other'
        FILTER_CONTRTOL_NAME='Select 2016/03/10 and 2016/03/17'
        ...
        EXPECTED_DEFAULT_DATE_FILTER1_VALUE=['Feb 11, 2016']
        EXPECTED_DEFAULT_DATE_FILTER2_VALUE=['Feb 18, 2016']
        EXPECTED_DEFAULT_DATE_RANGE_FILTER_VALUE=['Feb 11, 2016 - Feb 18, 2016']
        EXPECTED_CONTAINER_BUTTONS=['Maximize', 'Options']
        EXPECTED_PAGE_HEADER_BUTTONS=['Refresh', 'Show filters']
        EXPECTED_CONTAINERS=['18 - Date Range Optional Other', 'Panel 2', 'Panel 3', 'Panel 4', 'Panel 5', 'Panel 6', 'Panel 7', 'Panel 8', 'Panel 9']
        ...
        EXPECTED_DATA_TAB_PROPERTIES=['Default value=February 11 2016', 'Default value=February 18 2016']
        ...
        """
            STEP 04 : Drag "18 - Date Range Optional Other" report into Panel 1 from P292_S10660 > G433312 > Reference Items folder.
        """
        ...
        pd_design.wait_for_visible_text("div[data-ibx-type='pdFilterPanel'] div[class='ibx-label-text']", visble_element_text='Select 2016/03/10 and 2016/03/17', time_out=SHORT_TIME, pause_time=4)
        
        """
            STEP 05.1 : Verify date filter controls bounded to the page with default values and red dotted border.
        """
        STEP5_EXPECTED_FILTER_CONTROLS=['Select 2016/03/10 and 2016/03/17', 'Select End Date 2016/03/17:']
        ...
        pd_design.verify_selected_date_in_filter_date_picker(STEP5_EXPECTED_FILTER_CONTROLS[0], EXPECTED_DEFAULT_DATE_FILTER1_VALUE, 'Step 05.06 : Verify {0} date selected as default in first date picker control'.format(EXPECTED_DEFAULT_DATE_FILTER1_VALUE))
        pd_design.verify_selected_date_in_filter_date_picker(STEP5_EXPECTED_FILTER_CONTROLS[1], EXPECTED_DEFAULT_DATE_FILTER2_VALUE, 'Step 05.07 : Verify {0} date selected as default in second date picker control'.format(EXPECTED_DEFAULT_DATE_FILTER2_VALUE))
        ...
        """
            STEP 06 : Right click filter controls (right click over filter control label or calendar control area else grid cell will be selected) and select Combine.
        """
        pd_design.select_filter_control_context_menu(STEP5_EXPECTED_FILTER_CONTROLS[0], 'Combine')
        
        """
            STEP 06.1 : Verify date range control created and default values are retained.
        """
        EXPECTED_FILTER_CONTROLS=['Select 2016/03/10 and 2016/03/17']
        ...
        pd_design.verify_selected_date_in_filter_date_picker(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_DATE_RANGE_FILTER_VALUE, 'Step 06.04 : Verify {0} date selected as default in date range picker control'.format(EXPECTED_DEFAULT_DATE_RANGE_FILTER_VALUE))
        ...
        """
            STEP 08.1 : Verify page loads successfully with default value Date Range control.
        """
        ...
        pd_preview.verify_selected_date_in_filter_date_picker(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_DATE_RANGE_FILTER_VALUE, 'Step 08.07 : Verify {0} date selected as default in date range picker control'.format(EXPECTED_DEFAULT_DATE_RANGE_FILTER_VALUE))
        ...
        pd_preview.verify_html_report_data_set(DATA_SET_NAME1, 'Step 08.11 : Verify page loads successfully with default value Date Range control')
        pd_preview.switch_to_default_page()
        
        """    
            STEP 09 : Click Clear in Date range control.
        """
        pd_preview.clear_filter_date_value(FILTER_CONTRTOL_NAME)
            
        """
            STEP 10 : Select "Mar 10, 2016 - Mar 17, 2016" using Date Range calendar control.
        """
        ...
        pd_preview.verify_html_report_data_set(DATA_SET_NAME2, 'Step 10.01 : Verify selected filter condition applied in the page')
        pd_preview.switch_to_default_page()
        pd_preview.verify_filter_date_picker_is_optional(FILTER_CONTRTOL_NAME, 'Step 10.02 : Verify solid red border removed from date filter control')
        pd_preview.verify_selected_date_in_filter_date_picker(FILTER_CONTRTOL_NAME, EXPECTED_SELECTED_DATE, 'Step 10.03 : Verify {0} data selected in filter date range picker'.format(EXPECTED_SELECTED_DATE))
        ...
        """
            STEP 13.1 : Verify page runs successfully with default value Date Range control.
        """
        ...
        pd_run.verify_filter_control_labels(EXPECTED_FILTER_CONTROLS, 'Step 13.03 : Verify filter date range picker controls display in run page')
        pd_run.verify_filter_date_picker_is_optional(FILTER_CONTRTOL_NAME, 'Step 13.04 : Verify date filter control is optional')
        pd_run.verify_selected_date_in_filter_date_picker(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_DATE_RANGE_FILTER_VALUE, 'Step 13.05 : Verify {0} date selected as default in date range picker control'.format(EXPECTED_DEFAULT_DATE_RANGE_FILTER_VALUE))
        pd_run.verify_containers_title(EXPECTED_CONTAINERS, 'Step 13.06 : Verify {0} containers display in run page'.format(EXPECTED_CONTAINERS))
        pd_run.verify_container_title_bar_visible_buttons(CONTAINER_ITEM, EXPECTED_CONTAINER_BUTTONS, "Step 13.07 : Verify 'Maximize', 'Options' buttons display on {0} container tool bar in run page".format(CONTAINER_ITEM))
        pd_run.verify_number_of_panels(EXPECTED_TOTAL_PANELS, 'Step 13.08 : Verify {0} panels display in run page'.format(EXPECTED_TOTAL_PANELS))
        ...
        pd_run.verify_html_report_data_set(DATA_SET_NAME1, 'Step 13.09 : Verify page runs successfully with Date Range filter control')
        pd_run.switch_to_default_page()
        pd_run.swtich_to_homepage_runwindow_frame()
        
        """
            STEP 14 : Click Clear in Date range control.
        """
        pd_run.clear_filter_date_value(FILTER_CONTRTOL_NAME)
        
        """
            STEP 15 : Select "Mar 10, 2016 - Mar 17, 2016" using Date Range calendar control.
        """    
        pd_run.select_date_from_date_range_picker(FILTER_CONTRTOL_NAME, START_DATE_TO_SELECT, END_DATE_TO_SELECT)
        
        """
            STEP 15.1 : Verify selected filter condition applied in the page.
        """
        pd_run.verify_selected_date_in_filter_date_picker(FILTER_CONTRTOL_NAME, EXPECTED_SELECTED_DATE, 'Step 15.01 : Verify {0} data selected in filter date range picker'.format(EXPECTED_SELECTED_DATE))
        pd_run.switch_to_container_frame(CONTAINER_ITEM)
        ...
        pd_run.verify_html_report_data_set(DATA_SET_NAME2, 'Step 15.02 : Verify selected filter condition applied in the page.')
        pd_run.switch_to_default_page()
        ...
        """
            STEP 17 : Delete the created Page.
        """
