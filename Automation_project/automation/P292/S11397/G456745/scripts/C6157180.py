...
Test Case Link  =  http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/6157180
...
class C6157180_TestClass(BaseTestCase):

    def test_C6157180(self):
        ...
        """
            STEP 05.1 : Verify filter controls bounded to the page with solid red border and red dotted border around it.
        """
        pd_design.verify_filter_control_labels(EXPECTED_FILTER_CONTROLS, 'Step 05.01 : Verify {0} filter control added'.format(EXPECTED_FILTER_CONTROLS))
        pd_design.verify_filter_control_panel_is_selected(FILTER_CONTRTOL_NAME, 'Step 05.02 : Verify first {0} filter control bounded to the page with red dotted border around it'.format(FILTER_CONTRTOL_NAME))
        pd_design.verify_filter_control_panel_is_selected(FILTER_CONTRTOL_NAME, 'Step 05.03 : Verify second {0} filter control bounded to the page with red dotted border around it'.format(FILTER_CONTRTOL_NAME), 2)
        pd_design.verify_filter_inputbox_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 05.04 : Verify first {0} filter control input box bounded to the page with solid red border'.format(FILTER_CONTRTOL_NAME))
        pd_design.verify_filter_inputbox_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 05.06 : Verify second {0} filter control input box bounded to the page with solid red border'.format(FILTER_CONTRTOL_NAME), 2)
        pd_design.verify_filter_inputbox_value(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_FILTER_CONTROL_VALUE, 'Step 05.07 : Verify first {0} filter control default input value is None'.format(FILTER_CONTRTOL_NAME))
        pd_design.verify_filter_inputbox_value(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_FILTER_CONTROL_VALUE, 'Step 05.08 : Verify second {0} filter control default input value is None'.format(FILTER_CONTRTOL_NAME), 2)
        ...
        """
            STEP 07.1 : Verify filter controls required state (solid red border around the control) are retained in page preview.
        """
        ...
        pd_preview.verify_filter_control_panel_is_not_selected(FILTER_CONTRTOL_NAME, 'Step 07.05 : Verify first {0} filter control not bounded to the page with red dotted border around it in preview'.format(FILTER_CONTRTOL_NAME))
        pd_preview.verify_filter_control_panel_is_not_selected(FILTER_CONTRTOL_NAME, 'Step 07.06 : Verify second {0} filter control not bounded to the page with red dotted border around it in preview'.format(FILTER_CONTRTOL_NAME), 2)
        pd_preview.verify_filter_inputbox_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 07.07 : Verify first {0} filter control input box is box bounded to the page with solid red border in preview'.format(FILTER_CONTRTOL_NAME))
        pd_preview.verify_filter_inputbox_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 07.08 : Verify second {0} filter control input box is box bounded to the page with solid red border in preview'.format(FILTER_CONTRTOL_NAME), 2)
        pd_preview.verify_filter_inputbox_value(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_FILTER_CONTROL_VALUE, 'Step 07.09 : Verify first {0} filter control default input value is None in preview'.format(FILTER_CONTRTOL_NAME))
        pd_preview.verify_filter_inputbox_value(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_FILTER_CONTROL_VALUE, 'Step 07.10 : Verify second {0} filter control default input value is None in preview'.format(FILTER_CONTRTOL_NAME), 2)
        ...
        pd_preview.verify_container_title_bar_visible_buttons(CONTAINER_ITEM, EXPECTED_CONTAINER_BUTTONS, "Step 07.11 : Verify 'Maximize', 'Options' buttons display on {0} container tool bar in preview".format(CONTAINER_ITEM))
        pd_preview.verify_number_of_panels(EXPECTED_TOTAL_PANELS, 'Step 07.12 : Verify 1 panel display in preview')
        pd_preview.switch_to_container_frame(CONTAINER_ITEM)
        pd_preview.verify_html_report_does_not_have_vertical_scrollbar('Step 07.13 : Verify container report does not have vertical scroll bar in preview')
        pd_preview.verify_html_report_does_not_have_horizontal_scrollbar('Step 07.14 : Verify container report does not have horizontal scroll bar in preview')
        pd_preview.switch_to_default_page()
        pd_preview.verify_default_output_for_input_required_container(CONTAINER_ITEM, 'Step 07.15 : Verify report display in preview')
        ...
        """
            STEP 10.1 : Verify filter controls are in required state (solid red border available around the filter control) at run time of the page.
        """
        ...
        pd_run.verify_filter_inputbox_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 10.04 : Verify first {0} filter control input box is box bounded to the page with solid red border in run page'.format(FILTER_CONTRTOL_NAME))
        pd_run.verify_filter_inputbox_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 10.05 : Verify second {0} filter control input box is box bounded to the page with solid red border in run page'.format(FILTER_CONTRTOL_NAME), 2)
        pd_run.verify_filter_inputbox_value(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_FILTER_CONTROL_VALUE, 'Step 10.06 : Verify first {0} filter control default input value is None in run page'.format(FILTER_CONTRTOL_NAME))
        pd_run.verify_filter_inputbox_value(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_FILTER_CONTROL_VALUE, 'Step 10.07 : Verify second {0} filter control default input value is None in run page'.format(FILTER_CONTRTOL_NAME), 2)
        pd_run.verify_containers_title(EXPECTED_CONTAINERS, 'Step 10.08 : Verify {0} containers display in run page'.format(EXPECTED_CONTAINERS))
        pd_run.verify_container_title_bar_visible_buttons(CONTAINER_ITEM, EXPECTED_CONTAINER_BUTTONS, "Step 10.09 : Verify 'Maximize', 'Options' buttons display on container tool bar in run page")
        pd_run.verify_number_of_panels(EXPECTED_TOTAL_PANELS, 'Step 10.10 : Verify 1 panel display in run page')
        pd_run.switch_to_container_frame(CONTAINER_ITEM)
        pd_run.verify_html_report_does_not_have_vertical_scrollbar('Step 10.11 : Verify container report does not have vertical scroll bar in run page')
        pd_run.verify_html_report_does_not_have_horizontal_scrollbar('Step 10.12 : Verify container report does not have horizontal scroll bar in run page')
        pd_run.switch_to_default_page()
        pd_run.swtich_to_homepage_runwindow_frame()
        pd_run.verify_default_output_for_input_required_container(CONTAINER_ITEM, 'Step 10.13 : Verify blank report display in run page', expected_output='')
