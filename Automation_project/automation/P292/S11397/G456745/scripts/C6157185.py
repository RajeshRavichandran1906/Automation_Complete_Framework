...
Test Case Link  =  http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/6157185
Test Case Title =  Adding optional other date control to the Page
-----------------------------------------------------------------------------------------------"""

from common.lib.basetestcase import BaseTestCase
from common.wftools import page_designer

class C6157185_TestClass(BaseTestCase):

    def test_C6157185(self):
        ...
        EXPECTED_DATA_TAB_PROPERTIES=['Default value=April 4 2016']
        ...
        """
            STEP 04 : Drag "15 - Date Optional Other" report into the page canvas from P292_S10660 > G433312 > Reference Items folder..
        """
        ...
        """
            STEP 05.1 : Verify date filter control bounded to the page with a default value and red dotted border.
        """
        ...
        pd_design.verify_filter_date_picker_is_optional(FILTER_CONTRTOL_NAME, 'Step 05.03 : Verify date filter control is optional')
        pd_design.verify_selected_date_in_filter_date_picker(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_DATE_FILTER_VALUE, 'Step 05.04 : Verify {0} selected as default in date filter control'.format(EXPECTED_DEFAULT_DATE_FILTER_VALUE))
        ...
        pd_preview.verify_filter_date_picker_is_optional(FILTER_CONTRTOL_NAME, 'Step 07.06 : Verify date filter control is optional')
        pd_preview.verify_selected_date_in_filter_date_picker(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_DATE_FILTER_VALUE, 'Step 07.07 : Verify {0} selected as default in date filter control'.format(EXPECTED_DEFAULT_DATE_FILTER_VALUE))
        ...
        pd_preview.verify_html_report_data_set(DATA_SET_NAME1, 'Step 07.11 : Verify page loads successfully with default value filter condition')
        pd_preview.switch_to_default_page()
        
        """
            STEP 08 : Select "Mar 17, 2016" using Calendar control.
        """
        pd_preview.select_date_from_single_date_picker(FILTER_CONTRTOL_NAME, month=MONTH_TO_SELECT, day=DAY_TO_SELECT)
        
        """
            STEP 08.1 : Verify selected filter condition applied in the page.
        """
        pd_preview.switch_to_container_frame(CONTAINER_ITEM)
        pd_preview.wait_for_visible_text("table[summary]>tbody>tr:nth-child(2)>td:last-child", visble_element_text='282', time_out=LONG_TIME)
        #pd_preview.create_html_report_data_set(DATA_SET_NAME2)
        pd_preview.verify_html_report_data_set(DATA_SET_NAME2, 'Step 08.03 : Verify filter condition applied in the page')
        pd_preview.switch_to_default_page()
        ...
        """
            STEP 11.1 : Verify page loads successfully with default value filter condition..
        """
        ...
        pd_run.verify_filter_date_picker_is_optional(FILTER_CONTRTOL_NAME, 'Step 11.04 : Verify date filter control is optional')
        pd_run.verify_selected_date_in_filter_date_picker(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_DATE_FILTER_VALUE, 'Step 11.05 : Verify {0} selected as default in date filter control'.format(EXPECTED_DEFAULT_DATE_FILTER_VALUE))
        ...
        pd_run.verify_html_report_data_set(DATA_SET_NAME1, 'Step 11.08 : Verify page loads successfully with Date filter control in preview')
        ...
        """    
        pd_run.select_date_from_single_date_picker(FILTER_CONTRTOL_NAME, month=MONTH_TO_SELECT, day=DAY_TO_SELECT)
        
        """
            STEP 12.1 : Verify selected filter condition applied in the page.
        """
        ...
        pd_run.switch_to_container_frame(CONTAINER_ITEM)
        pd_run.wait_for_visible_text("table[summary]>tbody>tr:nth-child(2)>td:last-child", visble_element_text='282', time_out=LONG_TIME)
        pd_run.verify_html_report_data_set(DATA_SET_NAME2, 'Step 12.03 : Verify filter condition applied in the page')
        pd_run.switch_to_default_page()
