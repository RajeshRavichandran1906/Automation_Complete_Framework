...
Test Case Link  =  http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/6157192
Test Case Title =  Adding required dynamic single-select dropdown control to the Page
-----------------------------------------------------------------------------------------------"""
...
class C6157192_TestClass(BaseTestCase):

    def test_C6157192(self):
        ...
        """
            STEP 07.1 : Verify page loads successfully in preview with solid red border around the filter control.
        """
        ...
        pd_preview.verify_filter_dropdown_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 07.06 : Verify {0} drop down control bounded to the page with red solid border around it in preview'.format(FILTER_CONTRTOL_NAME))
        ...
        pd_preview.verify_default_output_for_input_required_container(CONTAINER_ITEM, 'Step 07.11 : Verify page loads successfully in preview')
       
        """
            STEP 08 : Select "Oceania" value in dropdown control.
        """
        FILTER_DROPDOWN_SELECTION_OPTION='Oceania'
        pd_preview.select_filter_dropdown_option(FILTER_CONTRTOL_NAME, FILTER_DROPDOWN_SELECTION_OPTION)
        
        """
            STEP 08.1 : Verify solid red border not available over the filter control and selected filter condition applied in the page.
        """
        pd_preview.verify_selected_value_of_filter_dropdown(FILTER_CONTRTOL_NAME, [FILTER_DROPDOWN_SELECTION_OPTION], 'Step 08.01 : Verify {0} option selected in filter drop down'.format(FILTER_DROPDOWN_SELECTION_OPTION))
        pd_preview.verify_filter_dropdown_is_optional(FILTER_CONTRTOL_NAME, 'Step 08.02 : Verify solid red border not available over the filter control')
        ...
        pd_preview.verify_html_report_data_set(DATA_SET_NAME1, 'Step 08.03 : Verify selected filter condition applied in the page')
        ...
        """
            STEP 11.1 : Verify page runs successfully with required filter control (solid red border around the filter control).
        """
        ...
        pd_run.verify_filter_dropdown_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 11.04 : Verify solid red border available over the filter control in run page')
        ...
        pd_run.verify_default_output_for_input_required_container(CONTAINER_ITEM, 'Step 11.09 : Verify page runs successfully')
        pd_run.swtich_to_homepage_runwindow_frame()
        
        """
            STEP 12 : Click dropdown control press ctrl and multi-select "North America" and "South America" values.
        """
        ...
        """
            STEP 12.1 : Verify can't able to multi-select values in single-select dropdown control.
        """
        utils.asequal(self, False, multi_option, 'Step 12.01 : Verify can not able to multi-select values in single-select dropdown control')
        pd_preview.verify_selected_value_of_filter_dropdown(FILTER_CONTRTOL_NAME, [MULTI_OPTIONS[0]], 'Step 12.02 : Verify user can select only one option from filter drop down')
        
        """
            STEP 13 : Click dropdown control and select "North America" value.
        """
        FILTER_DROPDOWN_SELECTION_OPTION='North America'
        pd_preview.select_filter_dropdown_option(FILTER_CONTRTOL_NAME, FILTER_DROPDOWN_SELECTION_OPTION)
        
        """
            STEP 13.1 : Verify filter condition applied in the page.
        """
        pd_preview.verify_selected_value_of_filter_dropdown(FILTER_CONTRTOL_NAME, [FILTER_DROPDOWN_SELECTION_OPTION], 'Step 13.01 : Verify {0} option selected in filter drop down'.format(FILTER_DROPDOWN_SELECTION_OPTION))
        pd_preview.switch_to_container_frame(CONTAINER_ITEM)
        ...
        pd_preview.verify_html_report_data_set(DATA_SET_NAME2, 'Step 13.02 : Verify filter condition applied in the page')
        pd_preview.switch_to_default_page()
