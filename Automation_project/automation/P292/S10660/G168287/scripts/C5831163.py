...
Testcase ID : 172.19.2.180/testrail/index.php?/cases/view/5831163
'''
...
class C5831163_TestClass(BaseTestCase):
    
    def test_C5831163(self):
        
        """   
            TESTCASE VARIABLES 
        """
        Test_Case_ID = 'C5831163'
        ...
        utillobj.synchronize_with_visble_text("#TableChart_1 text[class='legend-labels!s0!']", 'Series0', metaobj.home_page_medium_timesleep)
        ...
        """"
            Step 04 : Click "Data" tab > "Join" icon.
            Step 05 : Click "Add New" in the "Join" window.
            Step 06: Select "TRAINING.MAS" > Click "Open".
        """           
        iaribbon.create_join_to_handle_lessthan_two_targets('ibisamp->training', 'ibisamp', save_folder='ibisamp')
        ...
        """
            Step 07 :Verify a JOIN is created between the two master files.
        """
        actual_text_list=[join.text.strip() for join in self.driver.find_elements_by_css_selector(join_css)]
        utillobj.asequal(actual_text_list, ['ibisamp/empdata', 'ibisamp/training'], 'Step 07.01 : Verify a JOIN is created between the two master files.')
           
        """
            Step 08 : Click "OK".
        """
        iaribbon.select_join_menu_buttons('ok')
        utillobj.synchronize_with_visble_text("#iaMetaDataBrowser div[id^='QbMetaDataTree-'] tr:nth-child(13)>td[class='']", 'COURSECODE', metaobj.home_page_medium_timesleep)
           
        """
            Step 09 : Double click "LASTNAME", "SALARY", "EXPENSES".
        """
        metaobj.datatree_field_click('LASTNAME',2,1)
        utillobj.synchronize_with_visble_text("#TableChart_1 text[class='xaxisOrdinal-title']", 'LASTNAME', metaobj.home_page_medium_timesleep)
           
        metaobj.datatree_field_click('SALARY',2,1)
        parent_css="#pfjTableChart_1 [class*='riser']"
        utillobj.synchronize_with_number_of_element(parent_css, 1, metaobj.home_page_medium_timesleep)
   
        metaobj.datatree_field_click('EXPENSES',2,1)
        utillobj.synchronize_with_number_of_element(parent_css, 2, metaobj.home_page_medium_timesleep)
      
        """
            Step 10 : Verify the Streamgraph in "Live Preview" is updated to display the selected data.
        """
...
#         utillobj.take_screenshot(screenshot_element, Test_Case_ID+'_Actual_Step_15', 'actual')
        ...
        utillobj.synchronize_with_visble_text("#jschart_HOLD_0 text[class='xaxisOrdinal-title']", 'LASTNAME', metaobj.home_page_medium_timesleep)
           
        """
            Step 12 : Verify the same chart is displayed at run time.
        """
        verify_chart_output('jschart_HOLD_0', '18')
        resultobj.verify_number_of_riser('jschart_HOLD_0', 2, 39, 'Step 12.01 : Verify number of marker',custome_css=" circle[class^='marker']")
        utillobj.switch_to_default_content(3)
           
        """
            Step 13 : Click Save in the toolbar > Save as "C5831163" > Click Save
        """
        ...
        """
            Step 15: Launch the IA API to run fex from bip.
        ...
        utillobj.active_run_fex_api_login(Test_Case_ID+'.fex', 'S10660_chart_2',mrid='mrid', mrpass='mrpass')
        utillobj.synchronize_with_visble_text("#jschart_HOLD_0 text[class='xaxisOrdinal-title']", 'LASTNAME', 65)
         
        """
            Step 16 : Verify the Streamgraph is run in a new window.
        """
        resultobj.verify_number_of_riser('jschart_HOLD_0', 2, 39, 'Step 16.01 : Verify number of marker',custome_css=" circle[class^='marker']")
         
        """
            Step 17 : Hover over on chart and verify tooltip values
        """
        expected_list=['LASTNAME:MARTIN', 'EXPENSES:2,050.00']
        chart_obj.verify_tooltip_in_run_window('marker!s1!g23!mmarker!', expected_list, 'Step 17.01: tooltip verification', parent_css="#jschart_HOLD_0", yoffset=-20,use_marker_enable=True)
        ...
        """
            Step 19:Restore the fex using API (edit the domain, port and alias of the URL - do not use the URL as is):
