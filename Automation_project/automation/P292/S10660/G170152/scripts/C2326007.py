...
Test Case = http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/2326007
...
class C2326007_TestClass(unittest.TestCase):
    ...
    def test_C2326007(self):
        
        """
        TESTCASE VARIABLES
        """
        dev_username = 'mrid03'
        dev_password = 'mrpass03'
        long_wait = 120
        medium_wait = 60
        ...
        """
        wftools_login_obj.invoke_home_page(dev_username, dev_password)
        ...
        """
        Step02: In the banner link, click on the top right username > Tools > click ESRI Configuration Utility.
        ...
        3.Cancel
        """
        select_and_verify_new_tab('Tools->ESRI Configuration Utility', 'Step02.1', 'ESRI Configuration Utility',control_type='text')
        ...
        utillobj.verify_picture_using_sikuli(ESRI_editfile_image, "Step02.3: Verify ESRI Edit file image")
            
        """
        Step03: Close ESRI Configuration Utility window.
        """
        core_utilobj.switch_to_previous_window()
        if len(driver.window_handles) != 1:
            time.sleep(45)
        core_utilobj.update_current_working_area_browser_specification()
        utillobj.synchronize_with_number_of_element(folders_css, 1, medium_wait)
         
            
        """
        Step04: In the banner link, click on the top right username > Tools > click Deferred Status.
        Verify Deferred Report Status opens in a new tab.
        """
        select_and_verify_new_tab('Tools->Deferred Status', 'Step04.1', 'Deferred Report Status')
        ...
        utillobj.verify_object_visible(refresh_image_css, True, "Step04.1c: Verify Refresh image object is displaying")
        utillobj.verify_object_visible(sort_image_css, True, "Step04.1c: Verify sort image object is displaying")
        utillobj.verify_object_visible(delete_image_css, True, "Step04.1c: Verify delete image object is displaying")
        ...
        utillobj.as_List_equal(expected_text_list_2, actual_text_list_2, "Step04.2b: Verify text Refresh, Sort By, Delete, Help")
                  
        href = self.driver.find_elements_by_css_selector("td a[href]")
        actual_href_count = len(href)
        utillobj.asequal(4, actual_href_count, "Step04.2c: Verify hyperlink count in Deferred Status Report page")       
        ...
        utillobj.asequal("Date", actual_css, "Step04.2d: Verify Date selected in Sort By dropdown")
                  
        """
        Step05: Close Deferred Report Status tab.
        """
        core_utilobj.switch_to_previous_window()
        if len(driver.window_handles) != 1:
            time.sleep(45)
        ...
        """
        Step06: In the banner link, click on the top right username > Tools > click Stop Requests.
        ...
        utillobj.asequal("0.5", str(actual_background_layer), "Step06: Verify background gray layer when stop request dialog opens")
                  
        """
        Step07: Close Stop Requests dialog box.
        """
        ...
        """
        Step08: In the banner link, click on the top right username > Tools > click Session Viewer.
        Verify new tab opens and it display the user session viewer.
        """
        utillobj.synchronize_with_number_of_element(folders_css, 1, long_wait)
        select_and_verify_new_tab('Tools->Session Viewer', 'Step08.1', '.*Session Viewer.*')
        ...
        css ="#table1 tbody td.console-header"
        s = self.driver.find_elements_by_css_selector(css)
        actual_text_list= [el.text.replace(' ','').replace('\n','') for el in s if el.text!='']
        expected_text_list = ['UserName', 'TracingLevel', 'FexEchoSetting', 'SQLTracing', 'ViewingFile', 'SessionStarted', 'LastActivity', 'URLCount', 'AverageResponse', 'ServerRequests', 'AverageServerTime', 'AverageDBMSTime']
        utillobj.as_List_equal(expected_text_list, actual_text_list, "Step08.3.1: Verify labels in Session Viewer")
            
        select = self.driver.find_elements_by_css_selector("#table1 tbody select")
        utillobj.asequal(1,len(select),"Step08.3.2: Verify the count of dropdown(select tag) in Session Viewer")
        radio = self.driver.find_elements_by_css_selector("#table1 tbody [type='radio']")
        utillobj.asequal(5,len(radio),"Step08.3.3: Verify the count of radio buttons in Session Viewer")
            
        """
        Step09: Close Session viewer tab.
        """
        core_utilobj.switch_to_previous_window()
        if len(driver.window_handles) != 1:
            time.sleep(45)
        ...
        """
        Step10: In the banner link, click on the top right username > Tools > click ReportCaster Explorer.
        Verify ReportCaster Explore opens in a new tab.
        """
        utillobj.synchronize_with_number_of_element(folders_css, 1, long_wait)
        select_and_verify_new_tab('Tools->ReportCaster Explorer', 'Step10', 'ReportCaster Explorer')
        ...
        title= 'ReportCaster Explorer'; report_caster_image= 'report_caster.png'; step_no='Step10'
        ...
        utillobj.asequal("Organize", actual_organize_label['dom_visible_text'], "Step10.5a: Verify Organize text in ReportCaster Explorer")
              
        organize_drop_down_css ="#IbfsExplorerPane_btnOrganize [class*='drop-down-arrow']"
        utillobj.verify_object_visible(organize_drop_down_css, True, "Step10.5b: Verify Organize drop down exists")
        ...
        for i in expected_domain_list:
            utillobj.asin(i, actual_domain_list, "Step10.6: Verify Domains present")
        ...
        utillobj.as_List_equal(expected_header_list, actual_header_list, "Step10.7: Verify titles in ReportCaster Panel explorer")
              
        utillobj.verify_picture_using_sikuli('rc_filter.png', "Step10.8: Verify filter image in "+title+" tab")
        utillobj.verify_picture_using_sikuli('rc_folder_view.png', "Step10.9: Verify folder view image in "+title+" tab")
              
        """
        Step11: Close ReportCaster Explorer tab.
        """
        core_utilobj.switch_to_previous_window()
        if len(driver.window_handles) != 1:
            time.sleep(45)
        ...
        """
        Step12: In the banner link, click on the top right username > Tools > click ReportCaster Status.
        ...
        """
        select_and_verify_new_tab('Tools->ReportCaster Status', 'Step12.1', 'ReportCaster Status')
        ...
        title= 'ReportCaster Status'; job_status_image= 'rc_job_status.png'; step_no='Step12.1'
        ...
        utillobj.as_List_equal(expected_job_status, actual_job_status, "Step12.1.4: Verify Manage Job Status options")
              
        job_status_refresh_drop_down_css = "[id*='manageJobStatusRefresh'] [class*='drop-down-arrow']"
        utillobj.verify_object_visible(job_status_refresh_drop_down_css, True, "Step12.1.5: Verify Manage Job Status Refresh drop down exists")
        ...
        utillobj.verify_object_visible(job_status_checked_css, True, "Step12.1.6: Verify Manage Job Status button checked exists")
              
        job_status_disabled_count = self.driver.find_elements_by_css_selector("#RCConsoleTabPage_manageJobStatusVBox [disabled='true']")
        utillobj.asequal(8,len(job_status_disabled_count), "Step12.1.7: Verify Manage Job Status disabled buttons and labels count")
        ...
        utillobj.as_List_equal(expected_show_options, actual_show_options, "Step12.2.1: Verify show ribbon tabs options")
                    
        utillobj.verify_picture_using_sikuli('rc_job_log.png', "Step12.2.2: Verify Job Log image in "+title+" tab")
        utillobj.verify_picture_using_sikuli('rc_blackout.png', "Step12.2.3: Verify Blackout image in "+title+" tab")
        utillobj.verify_picture_using_sikuli('rc_execution_id.png', "Step12.2.4: Verify Execution IDs image in "+title+" tab")
        ...
        utillobj.as_List_equal(expected_actions_options, actual_actions_options, "Step12.3.1: Verify Actions ribbon tabs options")
        utillobj.verify_picture_using_sikuli('rc_refresh.png', "Step12.3.2: Verify Refresh image in "+title+" tab")
              
        """
        Step13: Close ReportCaster Status tab.
        """
        core_utilobj.switch_to_previous_window()
        if len(driver.window_handles) != 1:
            time.sleep(45)
        ...
        """
        Step14: In the banner link, click on the top right username > Tools > click Magnify Search Page.
        ...
        """
        utillobj.synchronize_with_number_of_element(folders_css, 1, long_wait)
        select_and_verify_new_tab('Tools->Magnify Search Page', 'Step14', 'Search Results:')
        search_result_css = "div input:not([type*='hidden'])"
        utillobj.synchronize_with_number_of_element(search_result_css, 2, medium_wait)        
               
        title= 'Search Results:';search_results_image= 'search_results_century.png';step_no='Step14.1'
        ...
        utillobj.verify_object_visible(search_text_box_css, True, "Step14.2: Verify search_text_box object is displaying")
               
        collection_list_box_css = "select#site>option[value='default_collection']"
        verify_htmltag_text(collection_list_box_css, "Century Electronics KB", "Step14.3: Verify Collection list box listed as drop down.")
        ...
        utillobj.asequal("Search", search_button, "Step14.4: Verify Search button")
               
        verify_btn_css="html input[name='btnG']";text_css="html #sf span:nth-child(1)"
        utillobj.verify_popup(verify_btn_css,'Step14.5a: Verify Magnify Search Page displays Results Per Page', popup_text_css=text_css, popup_text="Results Per Page")        
        results_per_page_num_css = "span>select>option[selected]"
        verify_htmltag_text(results_per_page_num_css, "10", "Step14.5b: Verify no. of pages as drop down")
               
        advance_search_css = "[title*='advance'][title*='search'][href], [title*='Advanced'][title*='Search'][href]"
        verify_htmltag_text(advance_search_css, "Advanced Search", "Step14.6a: Verify Advanced Search text and hyperlink")
        verify_htmltag_property(advance_search_css, 'text-decoration','underline', "Step06.6b: Verify Advanced Search  text underline")
        verify_color(advance_search_css, 'hyper_link_blue', "Step14.6b: Verify Advanced Search text color")
               
        sign_out_css = "[title='Sign Out'][href]"
        verify_htmltag_text(sign_out_css, "Sign Out", "Step14.7a: Verify Sign Out text and hyperlink")
        verify_htmltag_property(sign_out_css, 'text-decoration','underline', "Step14.7b: Verify Sign Out text underline")
        verify_color(sign_out_css, 'hyper_link_blue', "Step14.7c: Verify Sign Out text color")
               
        verify_btn_css="html a[title='Go to Magnify Search Home']";text_css="html #ddd > table > tbody > tr > td:nth-child(2) > div > b"
        utillobj.verify_popup(verify_btn_css,'Step14.8: Verify the element Search Tips is Visible as expected.', popup_text_css=text_css, popup_text="Search Tips")
               
        """
        Step15: Close "Search Results:" tab.
        """
        core_utilobj.switch_to_previous_window()
        if len(driver.window_handles) != 1:
            time.sleep(45)
        ...
        """
        Step16: In the banner link, click on the top right username > Preferences.
        ...
        title= 'WebFOCUS Home Page'; step_no='Step16'; text_css="[class*='dialog-main-box'] [class*='ibx-label'][class*='label-text']"
        ...
        background_css = "[class*='ibx-popup-glass-pane']"
        actual_background_layer = self.driver.find_element_by_css_selector(background_css).value_of_css_property('opacity')
        utillobj.asequal("0.5", str(actual_background_layer), "Step16: Verify background gray layer when preferences dialog opens")
              
        """
        Step17: Close Preferences dialog box.
        """
        close_button_ele = self.driver.find_element_by_css_selector("[class*='dialog-main-box'] [class*='close-button']")
        core_utilobj.left_click(close_button_ele)
            
        """
        Step 18: In the banner link, click on the top right username > Help > click Information Center.
        Verify WebFOCUS Information Center opens in a new tab with following options:
        1.Learn.
        2.Connect.
        3.Technical Support.
        """
        select_and_verify_new_tab('Help->Information Center', 'Step 18.1', 'WebFOCUS Release.*')
        verify_title_and_tabs('Information Center', "Step 18.2")
        ...
        utillobj.as_List_equal(expected_list, actual_text_items, "Step 18.3: Verify Learn, Connect, Technical Support in Information Center")
            
        """
        Step 19: Close Information Center tab.
        """
        core_utilobj.switch_to_previous_window()
        if len(driver.window_handles) != 1:
            time.sleep(45)
        ...
        """
        Step 20: In the banner link, click on the top right username > Help > click VideoAssist.
        Verify a new tab should opens with links to informational videos.
        """
        select_and_verify_new_tab('Help->VideoAssist', 'Step 20', 'Videos')
        verify_title_and_tabs('Videos', "Step 20.1c")
            
        videos = self.driver.find_elements_by_css_selector("[href*='.mp4']")
        utillobj.as_LE(207,len(videos),"Step 20.2: Verify .mp4 video files in VideoAssist page")
            
        """
        Step 21: Close VideoAssist tab.
        """
        core_utilobj.switch_to_previous_window()
        if len(driver.window_handles) != 1:
            time.sleep(45)
        ...
        """
        Step 22: In the banner link, click on the top right username > Help > click Technical Library.
        Verify WebFOCUS Technical Content opens in a new tab.
        """
        select_and_verify_new_tab('Help->Technical Library', 'Step 22', '.*Technical Content.*')
        verify_title_and_tabs('Technical Content', "Step 22.1c")
            
        verify_btn_css="html #magnify-search-2 button"
        utillobj.verify_popup(verify_btn_css,'Step 22.2a: Verify Technical Content' , popup_text_css="html h1", popup_text="Technical Content")
            
        """
        Step 23: Close WebFOCUS Technical Content tab.
        """
        core_utilobj.switch_to_previous_window()
        if len(driver.window_handles) != 1:
            time.sleep(45)
        ...
        """
        Step 24: In the banner link, click on the top right username > Help > click Community.
        Verify a new tab should opens with links to different Information Builders Communities.
        """
        select_and_verify_new_tab('Help->Community', 'Step 24.1', 'Information Builders Community | Information Builders')
        verify_title_and_tabs('Information Builders Community | Information Builders', "Step 24.2")
            
        hyper_link = self.driver.find_elements_by_css_selector("[class*='columns small-12'] [href]")
        utillobj.as_GE(40,len(hyper_link),"Step 24.3: Verify hyper link references in Community page")
        ...
        utillobj.as_List_equal(expected_list, actual_text_items, "Step 24.4: m-card title in Community page")
            
        """
        Step 25: Close Information Builders Communities tab.
        """
        core_utilobj.switch_to_previous_window()
        if len(driver.window_handles) != 1:
            time.sleep(45)
        ...
        """
        Step 26: In the banner link, click on the top right username > Help > click Information Builders Home.
        Verify Business Intelligence and Data Management Software opens in a new tab.
        """
        select_and_verify_new_tab('Help->Information Builders Home', 'Step 26', 'Business Intelligence and Data Management Software*')
        verify_title_and_tabs('Business Intelligence and Data Management Software | Information Builders', "Step 26.1")
        ...
        expected=['Take Our 10-Minute Video Tour', "You'll Love the New WebFOCUS", 'Network, Learn and See a Demo', 'About Us', 'Support and Services']
        utillobj.as_List_equal(expected, actual_text_items, "Step 26.2: m-card title in Information Builders Home page")
            
        """
        Step 27: Close Business Intelligence and Data Management Software tab.
        """
        core_utilobj.switch_to_previous_window()
        if len(driver.window_handles) != 1:
            time.sleep(45)
        ...
        """
        Step 28: In the banner link, click on the top right username > Help > click About WebFOCUS.
        ...
        utillobj.asequal("About WebFOCUS", self.driver.find_element_by_css_selector(about_css).text, "Step 28: Verify dialog title")
        ...
        utillobj.as_List_equal(expected_left, actual_left_panel_items, "Step 28.1: Verify About dialog left menu")
        ...
        for item in right_items:
            if item.text != None:
                ...
                break
        if status== True:
            utillobj.asequal(True, status, "Step 28.2: Verify About page gen information verification.") 
             
        background_css = "[class*='ibx-popup-glass-pane']"
        actual_background_layer = self.driver.find_element_by_css_selector(background_css).value_of_css_property('opacity')
        utillobj.asequal("0.5", str(actual_background_layer), "Step 28.3: Verify background gray layer when About WebFOCUS dialog opens")
             
        """
        step 29: Click OK button to close "About WebFOCUS" dialog box.
        """
        close_button_ele = self.driver.find_element_by_css_selector("[class*='dialog-main-box'] [class*='close-button']")
        core_utilobj.left_click(close_button_ele)
            
        """
        Step 30: In the banner link, click on the top right username > Help > click Licenses.
        Verify 3rd Party Information opens in a new tab with a list of licenses.
        """
        select_and_verify_new_tab('Help->Licenses', 'Step 30.1', '3rd Party Information')
        verify_title_and_tabs('3rd Party Information', "Step 30.2")
        ...
        utillobj.as_List_equal(expected_item, actual_item, "Step 30.3: Verify labels in 3rd Party Information")
        ...
        for item in acutal_info:
            if 'Apache' in item:
                ...
                status_=False
        if status_== True:
            utillobj.asequal(True, status_, "Step 30.4: Verify 3rd Party Information verification.") 
        else:
            utillobj.asin('Apache', acutal_info, "Step 30.4: Verify 3rd Party Information verification.")
            
        """
        Step 31: Close 3rd Party Information tab.
        """
        core_utilobj.switch_to_previous_window()
        if len(driver.window_handles) != 1:
            time.sleep(45)
        ...
        """
        Step 32: In the banner link, click on the top right username > Legacy Home Page.
        Verify WebFOCUS Legacy Home opens in a new tab.
        """
        select_and_verify_new_tab('Legacy Home Page', 'Step 32.1', 'WebFOCUS Legacy Home')
        verify_title_and_tabs('WebFOCUS Legacy Home', "Step 32.2")
            
        domain_img_css = "[src*='discovery_domain']"
        utillobj.verify_object_visible(domain_img_css, True, "Step34.2: Verify domain image object is displaying")
            
        """
        Step 33: Close WebFOCUS Legacy Home.
        """
        core_utilobj.switch_to_previous_window()
        if len(driver.window_handles) != 1:
            time.sleep(45)
        ...
        """
        Step 34: In the banner link, click on the top right username > Sign Out.
        """
