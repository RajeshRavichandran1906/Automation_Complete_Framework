...
Test Case Link  =  http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/5852279
Test Case Title =  Adding optional static single-select button set control to the Page
-----------------------------------------------------------------------------------------------"""

from common.lib.basetestcase import BaseTestCase
from common.wftools import page_designer

class C5852279_TestClass(BaseTestCase):

    def test_C5852279(self):
        ...
        EXPECTED_FILTER_BUTTONSET_OPTIONS=['All', 'South America', 'EMEA', 'North America']
        EXPECTED_FILTER_BUTTONSET_SELECTED_OPTIONS=['All']
        ...
        """
            STEP 04 : Drag "20 - Single Select Static Optional" report into the page canvas from P292_S10660 > G433312 > Reference Items folder.
        """
        ...
        """
            STEP 07.1 : Verify dropdown control converted to button set control with "All" value selected.
        """
        ...
        pd_preview.verify_html_report_has_vertical_scrollbar('Step 09.12 : Verify vertical scroll bar available in report')
        pd_preview.scroll_down_html_report()
        pd_preview.verify_html_report_vertical_scrollbar_reached_bottom('Step 09.13 : Verify vertical scroll bar reached bottom of the report')
        pd_preview.verify_html_report_data_set(DATA_SET_NAME1, 'Step 09.14 : Verify page loads successfully with "All" filter condition')
        pd_preview.switch_to_default_page()
       
        """
            STEP 10 : Multi-select "EMEA" and "North America" values in button set control.
        """
        FILTER_BUTTONSET_MULTI_OPTIONS_TO_SELECT=['EMEA', 'North America']
        pd_preview.select_multiple_options_in_filter_buttonset(FILTER_CONTRTOL_NAME, FILTER_BUTTONSET_MULTI_OPTIONS_TO_SELECT)
        
        """
            STEP 10.1 : Verify unable to multi-select values in single select button set control.
        """
        EXPECTED_FILTER_BUTTONSET_SELECTED_MULTI_OPTIONS=['North America']
        pd_preview.verify_selected_options_in_filter_buttonset(FILTER_CONTRTOL_NAME, EXPECTED_FILTER_BUTTONSET_SELECTED_MULTI_OPTIONS, 'Step 10.01 : Verify unable to multi-select values in single select button set control')
        pd_preview.switch_to_container_frame(CONTAINER_ITEM)
        ...
        pd_preview.verify_html_report_data_set(DATA_SET_NAME2, 'Step 10.02 : Verify "North America" option only applied in report')
        pd_preview.switch_to_default_page()
        ...
        pd_run.verify_filter_buttonset_is_optional(FILTER_CONTRTOL_NAME, 'Step 13.04 : Verify {0} filter button is optional'.format(FILTER_CONTRTOL_NAME))
        ...
        pd_run.verify_selected_options_in_filter_buttonset(FILTER_CONTRTOL_NAME, EXPECTED_FILTER_BUTTONSET_SELECTED_OPTIONS, 'Step 13.06 : Verify {0} option selected as default in filter button set'.format(EXPECTED_FILTER_BUTTONSET_SELECTED_OPTIONS))
