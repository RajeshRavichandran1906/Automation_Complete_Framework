...
Test Case Link  =  http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/5852284
Test Case Title =  Adding required static multi-select button set control to the Page
-----------------------------------------------------------------------------------------------"""

from common.lib.basetestcase import BaseTestCase
from common.wftools import page_designer

class C5852284_TestClass(BaseTestCase):

    def test_C5852284(self):
        ...
        FILTER_BUTTONSET_MULTI_OPTIONS_TO_SELECT=['North America', 'EMEA']
        ...
        EXPECTED_CONTAINERS=['25 - Multi-Select Static Required', 'Panel 2', 'Panel 3']
        ...
        EXPECTED_GENERAL_TAB_PROPERTIES=['Type=Multiple button set', 'Tooltip=', 'Global name=']
        ...
        """
            STEP 04 : Drag "25 - Multi-Select Static Required" report into the Panel 1 from P292_S10660 > G433312 > Reference Items folder.
        """
        ...
        pd_design.verify_filter_buttonset_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 07.03 : Verify {0} filter button set control bounded to the page with solid red border around it'.format(FILTER_CONTRTOL_NAME))
        ...
        pd_design.verify_selected_options_in_filter_buttonset(FILTER_CONTRTOL_NAME, [], 'Step 07.05 : Verify no one option selected as default in filter button set')
        ...
        """
            STEP 08 : Right click filter control (right click over filter control label or button set, else grid cell will be selected) and select Settings.
        """
        ...
        pd_preview.verify_filter_buttonset_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 09.06 : Verify {0} filter button set control bounded to the page with solid red border around it in preview'.format(FILTER_CONTRTOL_NAME))
        ...
        pd_preview.verify_selected_options_in_filter_buttonset(FILTER_CONTRTOL_NAME, [], 'Step 09.08 : Verify no one option selected as default in filter button set')
        ...
        pd_preview.verify_number_of_panels(EXPECTED_TOTAL_PANELS, 'Step 09.11 : Verify {0} panel added in canvas'.format(EXPECTED_TOTAL_PANELS))
        ...
        pd_preview.verify_blank_container_output(EXPECTED_CONTAINERS[1], 'Step 09.13 : Verify {0} container is blank'.format(EXPECTED_CONTAINERS[1]))
        pd_preview.verify_blank_container_output(EXPECTED_CONTAINERS[2], 'Step 09.14 : Verify {0} container is blank'.format(EXPECTED_CONTAINERS[2]))
       
        """
            STEP 10 : Select "North America" value in button set control.
        """
        ...
        pd_preview.verify_blank_container_output(EXPECTED_CONTAINERS[1], 'Step 10.03 : Verify {0} container is blank'.format(EXPECTED_CONTAINERS[1]))
        pd_preview.verify_blank_container_output(EXPECTED_CONTAINERS[2], 'Step 10.04 : Verify {0} container is blank'.format(EXPECTED_CONTAINERS[2]))
        ...
        pd_run.verify_filter_buttonset_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 13.04 : Verify {0} filter button set control bounded to the page with solid red border around it in run page'.format(FILTER_CONTRTOL_NAME))
        ...
        pd_run.verify_selected_options_in_filter_buttonset(FILTER_CONTRTOL_NAME, [], 'Step 13.06 : Verify no one option selected as default in filter button set')
        ...
        pd_run.verify_number_of_panels(EXPECTED_TOTAL_PANELS, 'Step 13.09 : Verify 1 panel display in run page')
        ...
        """
            STEP 14 : Select "North America" and "EMEA" values in button set control.
        """
        ...
        """
        EXPECTED_FILTER_BUTTONSET_SELECTED_OPTIONS=['EMEA', 'North America']
        ...
        pd_run.swtich_to_homepage_runwindow_frame()
        pd_run.verify_blank_container_output(EXPECTED_CONTAINERS[1], 'Step 14.04 : Verify {0} container is blank'.format(EXPECTED_CONTAINERS[1]))
        pd_run.swtich_to_homepage_runwindow_frame()
        pd_run.verify_blank_container_output(EXPECTED_CONTAINERS[2], 'Step 14.05 : Verify {0} container is blank'.format(EXPECTED_CONTAINERS[2]))
        pd_run.swtich_to_homepage_runwindow_frame()
