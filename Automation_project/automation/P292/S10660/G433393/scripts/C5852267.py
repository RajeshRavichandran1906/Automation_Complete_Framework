...
Test Case Link  =  http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/5852267
Test Case Title =  Adding optional other static multi-select dropdown control to the Page
-----------------------------------------------------------------------------------------------"""

from common.lib.basetestcase import BaseTestCase
from common.wftools import page_designer

class C5852267_TestClass(BaseTestCase):

    def test_C5852267(self):
        ...
        DROPDOWN_OPTION_TO_SELECT = ['North America']
        ...
        EXPECTED_CONTAINERS=['27 - Multi-Select Static Optional Other']
        ...
        EXPECTED_GENERAL_TAB_PROPERTIES=['Type=Multiple select', 'Tooltip=', 'Global name=']
        ...
        """
            STEP 04 : Drag "27 - Multi-Select Static Optional Other" report into the page canvas from P292_S10660 > G433312 > Reference Items folder.
        """
        ...
        pd_design.verify_default_output_for_input_required_container(CONTAINER_ITEM, 'Step 04.07 : Verify report added to the page successfully', expected_output='An error has occurred')
        ...
        pd_preview.verify_html_report_data_set(DATA_SET_NAME1, 'Step 07.12 : Verify page loads successfully in preview with default value condition.')
        ...
        """
            STEP 10.1 : Verify page runs successfully with default value filter condition.
        """
        pd_run.verify_page_heading_title(EXPECTED_PAGE_HEADING, 'Step 10.01 : Verify page heading  in run page')
        ...
        pd_run.verify_filter_control_labels(EXPECTED_FILTER_CONTROLS, 'Step 10.03 : Verify {0} filter controls display in run page'.format(EXPECTED_FILTER_CONTROLS))
        pd_run.verify_filter_dropdown_is_optional(FILTER_CONTRTOL_NAME, 'Step 10.04 : Verify {0} drop down control is optional in run page'.format(FILTER_CONTRTOL_NAME))
        pd_run.verify_selected_value_of_filter_dropdown(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_FILTER_DROPDOWN_VALUE, 'Step 10.05 : Verify {0} option selected as default in filter drop down'.format(EXPECTED_DEFAULT_FILTER_DROPDOWN_VALUE))
        ...
        pd_run.verify_container_title_bar_visible_buttons(CONTAINER_ITEM, EXPECTED_CONTAINER_BUTTONS, "Step 10.07 : Verify 'Maximize', 'Options' buttons display on {0} container tool bar in run page".format(CONTAINER_ITEM))
        ...
        pd_run.verify_html_report_data_set(DATA_SET_NAME1, 'Step 10.09 : Verify page runs successfully with default value filter condition')
        pd_run.switch_to_default_page()
        pd_run.swtich_to_homepage_runwindow_frame()
        
        """
            STEP 11 : Click dropdown control and select "North America" value.
        """
        pd_run.verify_filter_dropdown_options(FILTER_CONTRTOL_NAME, EXPECTED_FILTER_DROPDOWN_OPTIONS, 'Step 11.01 : Verify {0} options are display in filter drop down'.format(EXPECTED_FILTER_DROPDOWN_OPTIONS))
        pd_run.select_multiple_options_from_filter_dropdown(FILTER_CONTRTOL_NAME, DROPDOWN_OPTION_TO_SELECT)
        pd_run.verify_selected_value_of_filter_dropdown(FILTER_CONTRTOL_NAME, EXPECTED_SELECTED_FILTER_DROPDOWN_VALUE, 'Step 11.02 : Verify {0} options are selected in filter drop down'.format(EXPECTED_SELECTED_FILTER_DROPDOWN_VALUE))
        
        """
            STEP 11.1 : Verify able to select multiple values.
        """
        pd_run.verify_selected_filter_dropdown_options(FILTER_CONTRTOL_NAME, EXPECTED_SELECTED_FILTER_DROPDOWN_OPTIONS, 'Step 11.03 : Verify able to select multiple values')
        pd_run.switch_to_container_frame(CONTAINER_ITEM)
        pd_run.wait_for_number_of_element("table[summary]>tbody>tr", expected_number=16, time_out=LONG_TIME)
        #pd_run.create_html_report_data_set(DATA_SET_NAME2)
        pd_run.verify_html_report_data_set(DATA_SET_NAME2, 'Step 11.04 : Verify multip option filter applied in report')
        pd_run.switch_to_default_page()
