...
Test Case Link  =  http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/5852278
Test Case Title =  Adding required static single-select button set control to the Page
-----------------------------------------------------------------------------------------------"""

from common.lib.basetestcase import BaseTestCase
from common.wftools import page_designer

class C5852278_TestClass(BaseTestCase):

    def test_C5852278(self):
        ...
        FILTER_BUTTONSET_OPTIONS_TO_SELECT='EMEA'
        ...
        EXPECTED_FILTER_BUTTONSET_SELECTED_OPTIONS=['EMEA']
        EXPECTED_SETTING_TABS=['General Settings', 'Control Settings', 'Data Settings', 'Parameters']
        EXPECTED_GENERAL_TAB_PROPERTIES=['Type=Single button set', 'Tooltip=', 'Global name=']
        ...
        pd_design.verify_filter_dropdown_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 05.03 : Verify {0} drop down control bounded to the page with solid red border around it'.format(FILTER_CONTRTOL_NAME))
        ...
        """
            STEP 07.1 : Verify dropdown control converted to button set control and solid red border retained over the control.
        """
        ...
        pd_design.verify_filter_control_panel_is_selected(FILTER_CONTRTOL_NAME, 'Step 07.02 : Verify {0} button set control bounded to the page with red dotted border around it'.format(FILTER_CONTRTOL_NAME))
        pd_design.verify_filter_buttonset_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 07.03 : Verify {0} button set control bounded to the page with solid red border around it'.format(FILTER_CONTRTOL_NAME))
        ...
        pd_design.verify_container_title_bar_visible_buttons(CONTAINER_ITEM, EXPECTED_CONTAINER_BUTTONS, "Step 07.05 : Verify 'Maximize', 'Options' buttons display on container tool bar")
        pd_design.verify_page_header_visible_buttons(EXPECTED_PAGE_HEADER_BUTTONS, 'Step 07.06 : Verify Refresh and Filter buttons are display on page header')
        ...
        """
            STEP 09.1 : Verify page loads successfully in preview with required button set control.
        """
        ...
        pd_preview.verify_filter_buttonset_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 09.06 : Verify {0} button set control bounded to the page with solid red border around it in preview'.format(FILTER_CONTRTOL_NAME))
        ...
        pd_preview.verify_containers_title(EXPECTED_CONTAINERS, 'Step 09.08 : Verify {0} containers display in preview'.format(EXPECTED_CONTAINERS))
        pd_preview.verify_container_title_bar_visible_buttons(CONTAINER_ITEM, EXPECTED_CONTAINER_BUTTONS, "Step 09.06 : Verify 'Maximize', 'Options' buttons display on {0} container tool bar in preview".format(CONTAINER_ITEM))
        pd_preview.verify_number_of_panels(EXPECTED_TOTAL_PANELS, 'Step 09.09 : Verify 1 panel display in preview')
        pd_preview.verify_default_output_for_input_required_container(CONTAINER_ITEM, 'Step 09.10 : Verify report loads successfully in preview')
       
        """
            STEP 10 : Select "EMEA" value in button set control.
        """
        pd_preview.select_filter_buttonset_option(FILTER_CONTRTOL_NAME, FILTER_BUTTONSET_OPTIONS_TO_SELECT)
        
        """
            STEP 10.1 : Verify solid red border over filter the control removed and selected condition applied in the page.
        """
        pd_preview.verify_selected_options_in_filter_buttonset(FILTER_CONTRTOL_NAME, EXPECTED_FILTER_BUTTONSET_SELECTED_OPTIONS, 'Step 10.01 : Verify {0} option selected in filter button set'.format(EXPECTED_FILTER_BUTTONSET_SELECTED_OPTIONS))
        pd_preview.verify_filter_buttonset_is_optional(FILTER_CONTRTOL_NAME, 'Step 10.02 : Verify solid red border over filter the control removed')
        ...
        """
            STEP 13.1 : Verify page runs successfully with required (solid red border around the control) button set control.
        """
        ...
        pd_run.verify_filter_buttonset_is_not_optional(FILTER_CONTRTOL_NAME, 'Step 13.04 : Verify {0} button set control bounded to the page with solid red border around it in run page'.format(FILTER_CONTRTOL_NAME))
        ...
        pd_run.verify_number_of_panels(EXPECTED_TOTAL_PANELS, 'Step 13.08 : Verify 1 panel display in run page')
        pd_run.verify_default_output_for_input_required_container(CONTAINER_ITEM, 'Step 13.09 : Verify report loads successfully in run page')
        pd_run.swtich_to_homepage_runwindow_frame()
        
        """
            STEP 14 : Select "North America" value in button set control.
        """
        FILTER_BUTTONSET_OPTIONS_TO_SELECT='North America'
        pd_run.select_filter_buttonset_option(FILTER_CONTRTOL_NAME, FILTER_BUTTONSET_OPTIONS_TO_SELECT)
        
        """
            STEP 14.1 : Verify selected condition applied in the page and solid red border removed from control.
        """
        EXPECTED_FILTER_BUTTONSET_SELECTED_OPTIONS=['North America']
        pd_run.verify_selected_options_in_filter_buttonset(FILTER_CONTRTOL_NAME, EXPECTED_FILTER_BUTTONSET_SELECTED_OPTIONS, 'Step 14.01 : Verify {0} option selected in filter button set'.format(EXPECTED_FILTER_BUTTONSET_SELECTED_OPTIONS))
        pd_run.verify_filter_buttonset_is_optional(FILTER_CONTRTOL_NAME, 'Step 14.02 : Verify solid red border over filter the control removed')
        ...
        pd_run.verify_html_report_data_set(DATA_SET_NAME2, 'Step 14.03 : Verify selected condition applied in the page')
