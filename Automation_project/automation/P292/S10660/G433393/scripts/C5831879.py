...
Test Case Link  =  http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/5831879
Test Case Title =  Adding optional other input filter control to the Page
-----------------------------------------------------------------------------------------------"""
from common.lib.basetestcase import BaseTestCase
from common.wftools import page_designer

class C5831879_TestClass(BaseTestCase):

    def test_C5831879(self):
        ...
        content_item_name='03 - Simple Input Optional Other'
        container_visible_buttons=['Maximize', 'Options']
        page_header_visible_buttons=['Refresh', 'Filter']
        default_filter_control_value=['Oceania']
        ...
        pd_design.verify_filter_control_labels(expected_filter_control, 'Step 05.2 : Verify Business Region filter condition only added')
        ...
        pd_design.verify_filter_inputbox_value(filter_control_name, default_filter_control_value, 'Step 05.6 : Verify Business Region filter condition default input value is Oceania')
        ...
        dat_settings=['Default value=Oceania']
        ...
        pd_design.switch_to_container_frame(content_item_name)
        pd_design.verify_html_report_data_set(TestCase_ID + '_DataSet_01', 'Step 07.2 : Verify default filter condition applied in the page.')
        pd_design.switch_to_default_page()
        pd_preview.verify_filter_control_labels(expected_filter_control, 'Step 07.3 : Verify filter controls.')
        ...
        pd_design.verify_filter_inputbox_value(filter_control_name, default_filter_control_value, 'Step 7.10 : Verify Business Region filter condition input value is Oceania')
        ...
        """
        pd_run.verify_filter_control_labels(expected_filter_control, 'Step 10.2 : Verify filter control is in required state in run page.')
        pd_run.verify_page_heading_title(page_heading, 'Step 10.3 : Verify page heading')
        pd_run.verify_containers_title(expected_containers, 'Step 10.4 : Verify canvas containers')
        ...
        pd_design.verify_filter_inputbox_value(filter_control_name, default_filter_control_value, 'Step 10.8 : Verify Business Region filter condition input value is Oceania')
        pd_design.switch_to_container_frame(content_item_name)
        pd_design.wait_for_visible_text("table[summary]>tbody>tr:nth-child(2)>td", visble_element_text='Oceania', time_out=max_time)
        pd_design.verify_html_report_data_set(TestCase_ID + '_DataSet_01', 'Step 10.8 : Verify page run successfully with "Oceania" filter condition.')
        pd_design.switch_to_default_page()
        ...
        """
        pd_run.swtich_to_homepage_runwindow_frame()
        pd_run.enter_input_value_for_filter_control(filter_control_name, input_value_to_enter='_FOC_NULL')
