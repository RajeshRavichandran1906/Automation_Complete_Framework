...
Test Case Link  =  http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/5852351
Test Case Title =  Adding optional dynamic multi-select button set control to the Page
-----------------------------------------------------------------------------------------------"""

from common.lib.basetestcase import BaseTestCase
from common.wftools import page_designer

class C5852351_TestClass(BaseTestCase):

    def test_C5852351(self):
        ...
        EXPECTED_CONTAINERS=['29 - Multi-Select Dynamic Optional', 'Panel 2', 'Panel 3']
        EXPECTED_FILTER_CONVERTER_CONTROLS=['Checkbox', 'Button set']
        EXPECTED_FILTER_BUTTONSET_OPTIONS=['All', 'EMEA', 'North America', 'Oceania', 'South America']
        ...
        """
            STEP 04 : Drag "29 - Multi-Select Dynamic Optional" report into Panel 1 from P292_S10660 > G433312 > Reference Items folder.
        """
        ...
        """
            STEP 07.1 : Verify dropdown control converted to button set control and "All" value is selected.
        """
        ...
        pd_preview.verify_html_report_has_vertical_scrollbar('Step 09.12 : Verify container report has vertical scroll bar in preview')
        pd_preview.scroll_down_html_report()
        pd_preview.verify_html_report_vertical_scrollbar_reached_bottom('Step 09.13 : Verify vertical scrollbar reached bottom of the report')
        pd_preview.verify_html_report_data_set(DATA_SET_NAME1, 'Step 09.14 : Verify all values are available')
        ...
        """
        FILTER_BUTTONSET_OPTION_TO_SELECT='EMEA'
        pd_run.select_filter_buttonset_option(FILTER_CONTRTOL_NAME, FILTER_BUTTONSET_OPTION_TO_SELECT)
        
        """
            STEP 10.1 : Verify selected filter condition applied in the page.
        """
        EXPECTED_FILTER_BUTTONSET_SELECTED_OPTIONS=[FILTER_BUTTONSET_OPTION_TO_SELECT]
        ...
        pd_preview.switch_to_container_frame(CONTAINER_ITEM)
        pd_preview.wait_for_number_of_element("table[summary]>tbody>tr", 9, time_out=SHORT_TIME)
        ...
        """
            STEP 11 : Unselect "EMEA" value in button set control.
        """
        FILTER_BUTTONSET_OPTION_TO_UNSELECT='EMEA'
        pd_run.select_filter_buttonset_option(FILTER_CONTRTOL_NAME, FILTER_BUTTONSET_OPTION_TO_UNSELECT)
        
        """
            STEP 11.1 : Verify "All" value selected in button set control and all values are available in the report.
        """
        pd_preview.verify_selected_options_in_filter_buttonset(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_FILTER_OPTION, 'Step 11.01 : Verify "All" value selected in button set control')
        pd_preview.verify_filter_buttonset_is_optional(FILTER_CONTRTOL_NAME, 'Step 11.02 : Verify solid red border not display over filter the control')
        pd_preview.switch_to_container_frame(CONTAINER_ITEM)
        pd_preview.wait_for_number_of_element("table[summary]>tbody>tr", 30, time_out=SHORT_TIME)
        pd_preview.verify_html_report_data_set(DATA_SET_NAME1, 'Step 11.03 : Verify all values are available in the report')
        pd_preview.switch_to_default_page()
        pd_preview.verify_blank_container_output(EXPECTED_CONTAINERS[1], 'Step 11.04 : Verify {0} container is blank'.format(EXPECTED_CONTAINERS[1]))
        pd_preview.verify_blank_container_output(EXPECTED_CONTAINERS[2], 'Step 11.05 : Verify {0} container is blank'.format(EXPECTED_CONTAINERS[2]))
        ...
        """
            STEP 13 : Save and Close the Designer.
        """
        pd_design.save_page_from_toolbar_with_default_name()
        pd_design.switch_to_previous_window()
        
        """
            STEP 14 : Run created Designer Page using right click menu option.
        """
        ...
        """
            STEP 14.1 : Verify page runs successfully with "All" filter condition.
        """
        pd_run.verify_page_heading_title(EXPECTED_PAGE_HEADING, 'Step 14.01 : Verify page heading  in run page')
        pd_run.verify_page_header_visible_buttons(EXPECTED_PAGE_HEADER_BUTTONS, 'Step 14.02 : Verify Refresh and Filter buttons are display on page header in run page')
        pd_run.verify_filter_control_labels(EXPECTED_FILTER_CONTROLS, 'Step 14.03 : Verify {0} filter controls display in run page'.format(EXPECTED_FILTER_CONTROLS))
        pd_run.verify_filter_buttonset_is_optional(FILTER_CONTRTOL_NAME, 'Step 14.04 : Verify {0} filter button set control is optional'.format(FILTER_CONTRTOL_NAME))
        pd_run.verify_filter_buttonset_options(FILTER_CONTRTOL_NAME, EXPECTED_FILTER_BUTTONSET_OPTIONS, 'Step 14.05 : Verify {0} options are display in filter button set control'.format(EXPECTED_FILTER_BUTTONSET_OPTIONS))
        pd_run.verify_selected_options_in_filter_buttonset(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_FILTER_OPTION, 'Step 14.06 : Verify {0} option selected as default in filter button set'.format(EXPECTED_DEFAULT_FILTER_OPTION))
        pd_run.verify_containers_title(EXPECTED_CONTAINERS, 'Step 14.07 : Verify {0} containers display in run page'.format(EXPECTED_CONTAINERS))
        pd_run.verify_container_title_bar_visible_buttons(CONTAINER_ITEM, EXPECTED_CONTAINER_BUTTONS, "Step 14.08 : Verify 'Maximize', 'Options' buttons display on {0} container tool bar in run page".format(CONTAINER_ITEM))
        pd_run.verify_number_of_panels(EXPECTED_TOTAL_PANELS, 'Step 14.09 : Verify {0} panel display in run page'.format(EXPECTED_TOTAL_PANELS))
        ...
        pd_run.verify_html_report_data_set(DATA_SET_NAME1, 'Step 14.10 : Verify page runs successfully with "All" condition')
        pd_run.switch_to_default_page()
        pd_run.swtich_to_homepage_runwindow_frame()
        pd_run.verify_blank_container_output(EXPECTED_CONTAINERS[1], 'Step 14.11 : Verify {0} container is blank'.format(EXPECTED_CONTAINERS[1]))
        pd_run.swtich_to_homepage_runwindow_frame()
        pd_run.verify_blank_container_output(EXPECTED_CONTAINERS[2], 'Step 14.12 : Verify {0} container is blank'.format(EXPECTED_CONTAINERS[2]))
        pd_run.swtich_to_homepage_runwindow_frame()
