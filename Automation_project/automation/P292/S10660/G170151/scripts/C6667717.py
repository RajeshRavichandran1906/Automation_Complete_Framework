...
Test Case Link  =  http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/6667717
Test Case Title =  AUTHZ_AdvancedUser:Check WebFOCUS Online Help and Change Password functionality for advanced user
-----------------------------------------------------------------------------------------------"""
...
class C6667717_TestClass(BaseTestCase):

    def test_C6667717(self):
        ...
        """
        main_page.change_password("password", "password3")
        ...
        """
            STEP 7 : Sign into WebFOCUS Home Page as Advanced User with the new password.
        """
        login.invoke_home_page('mridadvnew', 'mrpassadvnew')
        utils.synchronize_with_visble_text(locator.CONTENT_CSS, "Content", main_page.home_page_long_timesleep)
        ...
        """
            STEP 8 : Revert back to its Old Password by enter Values in,
            Old Password: password3 
            New Password: password
            Confirm New Password: password
            Click OK button.
            STEP 8.1 : Verify the window should disappear and displays a confirmation message "Your Password has been changed" with a background transparent green layer over the message.
        """
        main_page.select_username_dropdown_menu(navigate_path="Change Password")
        utils.synchronize_with_visble_text("div.change-old-password", "Password", main_page.home_page_medium_timesleep)
        
        main_page.change_password("password3", "password")
        ...
        """
            STEP 10 : Sign into WebFOCUS Home Page as Advanced User with the new password.
        """
        login.invoke_home_page('mridadv', 'mrpassadv')
        utils.synchronize_with_visble_text(locator.CONTENT_CSS, "Content", main_page.home_page_long_timesleep)
        
        """
            STEP 10.1 : Verify the password is revert back to its old password & WebFOCUS Home Page opens without any error.
        """
        content_obj = utils.validate_and_get_webdriver_object(locator.CONTENT_CSS, "Content css")
        actual_result = content_obj.is_displayed()
        msg = "Step 10.1 : Verify the password is revert back to its old password & WebFOCUS Home Page opens without any error."
        utils.asequal(True, actual_result, msg)
