...
@author: BM13368
TestCase ID : http://172.19.2.180/testrail/index.php?/cases/view/2235698
...
class C2235698_TestClass(BaseTestCase):

    def test_C2235698(self):
        
        Test_Case_ID = "C2235698"
        driver=self.driver
        ...
        """
        elem1=self.driver.find_element_by_css_selector("#singleReportCaptionLabel").get_property('text')
        ...
        menu_items=driver.find_elements_by_css_selector("div[id^='BiPopup'][style*='inherit'] table>tbody [class$='menu-item']")
        expected_popup_list=['Run', 'Preview', 'Run with Default Parameter Values']
        actual_popup_list=[el.text.strip() for el in menu_items  if bool(re.match('\S+', el.text.strip()))]
        print(actual_popup_list)
        utillobj.as_List_equal(expected_popup_list, actual_popup_list, "Step 13:01: Verify List of Values")
        
        """ 
            Step 14: Select Preview > Verify Preview output
        """
        ia_resultobj.verify_report_data_set('TableChart_1', 7, 2, Test_Case_ID+"_Ds03.xlsx", "Step 14:01: Verify live preview output")  
        """ 
