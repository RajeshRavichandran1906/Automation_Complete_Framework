...
#         utillobj.infoassist_api_edit(Test_Case_ID, 'idis', 'S10660_esrimap_2',mrid='mrid',mrpass='mrpass')
        ...
        """
        chart_type_css="#TableChart_1 svg>g>text.title"
        elem1=(By.CSS_SELECTOR, chart_type_css)
        resultobj._validate_page(elem1)
        expected_label_list = ['4000km', '2000mi']
        ...
        for label_x in expected_label_list:
            if label_x in next(my_iter_x):
                ...
                break
        del my_iter_x
        utillobj.asequal(statex, True, "Step 05: Verify the map canvas is changed")
        time.sleep(6)
        ...
        legend=['POPULATION', '0M', '9.1M', '18.2M', '27.3M', '36.4M']
        resultobj.verify_riser_legends("MAINTABLE_wbody1", legend, "Step32:d(i) Verify legend Title")
        utillobj.verify_chart_color("MAINTABLE_wbody1", "riser!s0!g1267!mregion!", "persian_red3", "Step 32.c(i) Verify first bar color")
        metaobj.verify_filter_pane_field('COUNTRY_United_States', 1, "Step 32")
        time.sleep(5)
        expected_tooltip=['STATE:Montana', 'POPULATION:47445', 'Filter Chart', 'Exclude from Chart']
        resultobj.verify_default_tooltip_values("MAINTABLE_wbody1","riser!s0!g1267!mregion!",expected_tooltip, "Step32.e: verify the default tooltip values")  
        time.sleep(5)
