...
class C2333393_TestClass(BaseTestCase):
    ...
    def test_C2333393(self):
        ...
        time.sleep(5)
        propertyobj.verify_slider_range_filter_prompts('#ar_Prompt_1','min','2011 Q1',msg="Step07: Verify filter prompt range min values")
        propertyobj.verify_slider_range_filter_prompts('#ar_Prompt_1','max','2017 Q1',msg="Step07: Verify filter prompt range max values")
        time.sleep(6)
        ...
        resultobj.wait_for_property(parent_css, 1)
        time.sleep(5)
        propertyobj.verify_slider_range_filter_prompts('#ar_Prompt_1','min','2013 Q4',msg="Step11: Verify filter prompt range min values")
        propertyobj.verify_slider_range_filter_prompts('#ar_Prompt_1','max','2017 Q1',msg="Step11: Verify filter prompt range max values")
        time.sleep(6)
        elem=(By.CSS_SELECTOR,'#MAINTABLE_wbody1')
        resultobj._validate_page(elem)
        parent_css="#MAINTABLE_wbody1 svg g text[class*='mgroupLabel']"
        ...
        parent_css= "#MAINTABLE_wbody1 svg g text[class*='yaxis-labels']"
        resultobj.wait_for_property(parent_css, 6)
        propertyobj.verify_slider_range_filter_prompts('#sliderPROMPT_1','min','2013 Q4',msg="Step13:Verify filter prompt range min values")
        propertyobj.verify_slider_range_filter_prompts('#sliderPROMPT_1','max','2017 Q1',msg="Step13: Verify filter prompt range max values")
        time.sleep(3)
        xaxis_value="Sale Year/Quarter"
        ...
        utillobj.take_screenshot(ele,'C2333393_Actual_step13', image_type='actual',x=1, y=1, w=-1, h=-1)
        ...
        resultobj.wait_for_property(parent_css, 6)
        time.sleep(3)
        propertyobj.verify_slider_range_filter_prompts('#ar_Prompt_1','min','2013 Q4',msg="Step19:Verify filter prompt range min values")
        propertyobj.verify_slider_range_filter_prompts('#ar_Prompt_1','max','2017 Q1',msg="Step19: Verify filter prompt range max values")
        time.sleep(3)
