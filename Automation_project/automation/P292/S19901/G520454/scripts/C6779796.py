...
Testcase ID : http://lnxtestrail.ibi.com/testrail//index.php?/cases/view/6779796
'''
...
class C6779796_TestClass(BaseTestCase):
    
    def test_C6779796(self):
        """
        Test_case variables
        """
        ...
        util_obj.set_text_field_using_actionchains(enter_user_in_inputbox, 'auto', keyboard_type=True)
        time.sleep(4)
        ...
        for item in users_list:
            if 'auto' in item.text:
                ...
                token=False
                break    
        util_obj.asequal(token, True, "Step 06a: Verify the list of users containing with a word 'auto' are displayed")
        token=False
        for item in users_list:
            if 'autobasuser08\nautobasuser08(auto@devmail.com)' in item.text and item.value_of_css_property('opacity')=='0.4':
                token=True
        util_obj.asequal(token, True, "Step 06b: Verify 'autobaseuser08' user is greyed out")
        ...
        util_obj.asequal(item_visibility_status, True, "Step 07: Verify that unable to select 'autobaseuser08' user")        
