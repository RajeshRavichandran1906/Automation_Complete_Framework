...
Test Case Link  =  http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/8320624
...
class C8320624_TestClass(BaseTestCase):

    def test_C8320624(self):
        ...
        pd_design.verify_container_title_bar_visible_buttons(CONTAINER_ITEM, EXPECTED_CONTAINER_BUTTONS, "Step 05.03 : Verify 'Maximize', 'Options' buttons display on container tool bar")
        pd_design.verify_page_header_visible_buttons(EXPECTED_PAGE_HEADER_BUTTONS, 'Step 05.04 : Verify Refresh and Filter buttons are display on page header')
        ...
        pd_preview.verify_containers_title(EXPECTED_CONTAINERS, 'Step 07.06 : Verify {0} containers display in preview'.format(EXPECTED_CONTAINERS))
        pd_preview.verify_container_title_bar_visible_buttons(CONTAINER_ITEM, EXPECTED_CONTAINER_BUTTONS, "Step 07.07 : Verify 'Maximize', 'Options' buttons display on {0} container tool bar in preview".format(CONTAINER_ITEM))
        pd_preview.verify_number_of_panels(EXPECTED_TOTAL_PANELS, 'Step 07.08 : Verify 1 panel display in preview')
