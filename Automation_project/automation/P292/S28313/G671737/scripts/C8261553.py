...
@author: Nasir
Test Case Link  =  http://172.19.2.180/testrail/index.php?/cases/view/8261553
...
class C8261553_TestClass(BaseTestCase):
    
    def test_C8261553(self):
        """    CLASS OBJECTS    """
        ...
        time.sleep(15) #this required for firefox to give some time to load
        
        """    2. Click Content View from the sidebar > Click on Workspaces from the resource tree    """
#         utils.synchronize_with_number_of_element(WfMainPageLocators.CONTENT_ICON_CSS, 1, 190)
        utils.wait_for_page_loads(wf_home.home_page_long_timesleep)
        ...
        utils.synchronize_with_visble_text('div .content-title-label', 'Items', 90)
        utils.wait_for_page_loads(wf_home.home_page_long_timesleep)
        ...
        """    Verify that user now in the content area under the place where the portal was created (Workspaces > P292_S19901 > G513445) and it is highlighted in blue and the domain/folders is in grey    """
        wf_home.verify_folders_in_grid_view(grid_list, 'asin', 'Step 04.01 : Verify the portal in grid view')
        wf_home.verify_crumb_box('Workspaces->{0}'.format(FOLDER_PATH), "Step 04.02")
        
        """    5. Right click on 'Portal for Context Menu T...'
        Verify that following Context Menu appears as same in the below screenshot    """
        wf_home.verify_repository_folder_item_context_menu(ITEM_NAME, EXPECTED_CONTEXTMENU_LIST, FOLDER_PATH,'Step 05.01')
