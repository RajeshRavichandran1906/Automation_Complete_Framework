...
#         metaobj.datatree_field_click('COUNTRY', 1, 1, 'Filter')
        ...
        utillobj.synchronize_with_visble_text('#qbFilterBox table>tbody', 'COUNTRY', 30)
        ...
        """
        status=driver.find_element_by_css_selector("#qbFilterBox div>table>tbody>tr>td>img[src*='remove']").is_displayed()
        utillobj.asequal(status, True, "Step 18:01: Verify pane filter icon is shown as exclude icon")
        """ 
            Step 19 :Logout:
            http://machine:port/ibi_apps/service/wf_security_logout.jsp
        """
