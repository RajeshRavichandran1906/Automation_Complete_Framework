...
Test Case = http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/2227671
Test case Name =  Filter Aggregation - Distinct Count
'''
...
class C2227671_TestClass(BaseTestCase):

    def test_C2227671(self):
        """
        TESTCASE VARIABLES
        """
        Test_Case_ID = 'C2227671'
        ...
        data_pane_items = ["Cost of Goods", "Product,Subcategory"]
        ...
        Step05: Click "Aggregation" dropdown menu > "Distinct Count"
        """
        metaobj.drag_drop_data_tree_items_to_filter("Cost of Goods", 1)  
        utillobj.synchronize_with_number_of_element("div[id^='QbDialog']", 1, metaobj.chart_medium_timesleep)
        elem = utillobj.validate_and_get_webdriver_object("#avAggregationComboBox", 'aggregation-box')     
        combo=['(None)', 'Sum', 'Average', 'Count', 'Distinct Count', 'Maximum', 'Minimum', 'Percent', 'Median']
        utillobj.select_any_combobox_item(elem,'Distinct Count',verify=True,expected_combobox_list=combo, msg="Step05: Verify list of agregation functions")
              
        """
        Step06: Verify Filter dialog
        Step07: Click OK
        """
        elem = utillobj.validate_and_get_webdriver_object("#avAggregationComboBox div", 'aggregation-combo-box')
        d=utillobj.get_attribute_value(elem,'text')
        utillobj.asequal(d['text'],'Distinct Count',"Step06: Verify Aggregation in Filter dialog")
        elem1 = utillobj.validate_and_get_webdriver_object("#avfFromValue input", 'value-from')
        d=utillobj.get_attribute_value(elem1, "int_value")
        utillobj.asequal(d['int_value'],"8","Step06: Verify From in Filter dialog")
        elem1 = utillobj.validate_and_get_webdriver_object("#avfToValue input", 'to-value')
        d=utillobj.get_attribute_value(elem1, "int_value")
        utillobj.asequal(d['int_value'],"71","Step06: Verify To in Filter dialog")
        ...
        """
        metaobj.verify_filter_pane_field('CNT.DST (Cost of Goods)',1,"Step08: Verify 'Distinct Count(Cost of Goods)' appears in filter pane")
        parent_css= "#ar_Prompt_1 span[id$='s_min']"
        resultobj.wait_for_property(parent_css, 1)
        parent_css= "#ar_Prompt_1 span[id$='s_max']"
        resultobj.wait_for_property(parent_css, 1)
        propertyobj.verify_slider_range_filter_prompts('#ar_Prompt_1','min',8,'int',msg="Step09: Verify filter prompt range values-min")
        propertyobj.verify_slider_range_filter_prompts('#ar_Prompt_1','max',71,'int',msg="Step09: Verify filter prompt range values-max")
        ...
        Step12: Hover over riser for value "Speaker Kits"
        Step13: Verify "Cost of Goods" value > $81,396,140.00
        """
        elem1=(By.CSS_SELECTOR, "rect[class*='riser!s']")
        resultobj._validate_page(elem1)
        elem=(By.CSS_SELECTOR,'#LOBJ1_cs')
        resultobj._validate_page(elem)
        parent_css="#MAINTABLE_wbody1 svg g text[class*='mgroupLabel']"
        resultobj.wait_for_property(parent_css, 21)
        parent_css= "#MAINTABLE_wbody1 svg g text[class*='yaxis-labels']"
        resultobj.wait_for_property(parent_css, 6) 
        propertyobj.verify_slider_range_filter_prompts('#LOBJ1_cs','min',8,'int',msg="Step11: Verify filter prompt range values-min")
        propertyobj.verify_slider_range_filter_prompts('#LOBJ1_cs','max',71,'int',msg="Step11: Verify filter prompt range values-max")
        xaxis_value="Product Subcategory"
        yaxis_value="Cost of Goods"
        ...
        bar=['Product Subcategory:Speaker Kits', 'Cost of Goods:$81,396,140.00', 'Filter Chart', 'Exclude from Chart', 'Drill up to Product Category', 'Drill down to Model']
        resultobj.verify_default_tooltip_values("MAINTABLE_wbody1", "riser!s0!g14!mbar", bar, "Step 12: Verify bar value")
        ...
        resultobj._validate_page(elem1)
        metaobj.verify_filter_pane_field('CNT.DST (Cost of Goods)',1,"Step19: Verify 'CNT.DST(Cost of Goods)' appears in filter pane")
        parent_css= "#ar_Prompt_1 span[id$='s_min']"
        resultobj.wait_for_property(parent_css, 1)
        parent_css= "#ar_Prompt_1 span[id$='s_max']"
        resultobj.wait_for_property(parent_css, 1)
        propertyobj.verify_slider_range_filter_prompts('#ar_Prompt_1','min',8,'int',msg="Step19: Verify filter prompt range values-min")
        propertyobj.verify_slider_range_filter_prompts('#ar_Prompt_1','max',71,'int',msg="Step19: Verify filter prompt range values-max")
        xaxis_value="Product Subcategory"
        yaxis_value="Cost of Goods"
