...
#         utillobj.infoassist_api_edit('test_base','idis','P292/S10032_visual_2', mrid = 'mrid', mrpass='mrpass')
...
#           
        ...
        expected_tooltip_values = ['Filter Chart', 'Exclude from Chart']
        browser = utillobj.parseinitfile('browser')
        if browser == 'Firefox':
            obtained_tooltip=self.select_default_tooltip_menu("MAINTABLE_wbody1", "riser!s0!g4!mstate",'Filter Chart', wait_time=1,expected_tool_tips=expected_tooltip)
            print("obtained tool is ",obtained_tooltip)
            utillobj.asequal(sorted(expected_tooltip_values),sorted(obtained_tooltip),"Step12: verify the tooltip values after click ")
 
        else:
            resultobj.select_default_tooltip_menu("MAINTABLE_wbody1", "riser!s0!g4!mstate",'Filter Chart',wait_time=1,default_move=True)
#         time.sleep(2)
#         resultobj.select_default_tooltip_menu("MAINTABLE_wbody1", "riser!s0!g4!mstate",'Filter Chart',wait_time=1)
    ...
    def select_default_tooltip_menu(self,parent_id, raiser_class, menu_path, wait_time=2,expected_tool_tips=[], **kwargs):
        utillobj = utillity.UtillityMethods(self.driver)
        if 'default_move' in kwargs:
            pass
        else:
            move1 = self.driver.find_element_by_css_selector("#"+ parent_id)
            utillobj.click_type_using_pyautogui(move1, move=True, **kwargs)
            time.sleep(3)
        ...
        if 'default_move' in kwargs:
            ...
            utillobj.click_type_using_pyautogui( obj_locator,**kwargs)
        utillobj.synchronize_with_number_of_element( tooltip_div_css, 1, 15, pause_time=0.3)
        tooltips=self.driver.find_elements_by_css_selector(tooltip_css)
        tooltip_text= [i.text.strip() for i in tooltips]
        utillobj.click_type_using_pyautogui( tooltips[0],**kwargs)
        for el in tooltips:
            if el.text.strip()==menus[0]:
                tooltip_obj = el
                break
        utillobj.default_click( tooltip_obj)
        return tooltip_text
