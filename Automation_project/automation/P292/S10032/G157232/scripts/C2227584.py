...
class C2227584_TestClass(BaseTestCase):
    ...
    def test_C2227584(self):
        ...
        resultobj.wait_for_property(query_tree, 1,20,string_value='CAR')
              
        metaobj.datatree_field_click('SEATS',2,1)
        resultobj.wait_for_property(query_tree, 1,20,string_value='SEATS')
             
        metaobj.datatree_field_click('SALES',2,1)
        resultobj.wait_for_property(query_tree, 1,20,string_value='SALES')
        ...
        """
        utillobj.save_file_from_browser(Test_Case_ID+'_Actual_DataSet_01', custom_cr_re="Untitled")
        utillobj.verify_xml_xls(Test_Case_ID+'_Base_DataSet_01.xlsx', Test_Case_ID+'_Actual_DataSet_01.xlsx', 'Step 08 : Verify it displays the data in Excel')
        if Global_variables.browser_name == 'edge' :
            utillobj.click_on_edge_browser_popup_button_using_uiautomation('Stay')
        else :
        ...
        """
        utillobj.wf_logout() # Changing logout function due to alert 
