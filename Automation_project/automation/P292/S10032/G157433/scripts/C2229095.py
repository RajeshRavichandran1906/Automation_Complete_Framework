...
from common.pages import visualization_metadata,visualization_resultarea, visualization_ribbon, ia_resultarea, ia_ribbon, metadata
...
class C2229095_TestClass(BaseTestCase):
    ...
    def test_C2229095(self):
        ...
        """
        utillobj.infoassist_api_login('idis','baseapp/dimensions/wf_retail_geography','P292/S10032_esrimap_2', 'mrid', 'mrpass')
        ...
        step05: Select baseapp/dimesntions/wf_retail_store.mas > "Open"
        """
        ia_ribbonobj.create_join("baseapp->dimensions->wf_retail_store", new_join=False)
        time.sleep(3)
        ...
        step08: Select baseapp/facts/wf_retail_sales.mas > "Open"
        """
        ia_ribbonobj.create_join("baseapp->facts->wf_retail_sales", new_join=False)
        time.sleep(3)
        ...
        resultobj.wait_for_property(parent_css,2,expire_time=60)   
        
        """
        Step12: Double click "Country", "Revenue"
        """
        new_meta.double_click_on_data_filed("Dimensions->Country",1)
        time.sleep(4) 
        parent_css="#MAINTABLE_wbody1 path[class^='riser']"
        utillobj.synchronize_with_number_of_element(parent_css, 41, 90, 1)
        
        metaobj.datatree_field_click("Measures/Properties->Revenue", 2,1)
        time.sleep(4)
        parent_css="#MAINTABLE_wbody1 path[class^='riser']"
        utillobj.synchronize_with_number_of_element(parent_css, 33, 90, 1)
        
        parent_css="#MAINTABLE_wbody1 .legend text"
        resultobj.wait_for_property(parent_css, 6)
        utillobj.synchronize_with_number_of_element(parent_css, 6, 30, 1)
...
#         utillobj.synchronize_with_number_of_element(chart_type_css, 1, 90, 1)
# #         elem1=(By.CSS_SELECTOR, chart_type_css)
# #         resultobj._validate_page(elem1)
        parent_css2="#MAINTABLE_wbody1 path[class^='riser']"
        utillobj.synchronize_with_number_of_element(parent_css2, 33, 90, 1)
...
#         resultobj._validate_page(elem)
        ...
        parent_css="#MAINTABLE_wbody1 .legend text"
        utillobj.synchronize_with_number_of_element(parent_css, 6, 60, 1)
