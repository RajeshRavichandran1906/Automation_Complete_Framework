'''
Created on Jan 02, 2018

@author: Praveen Ramkumar

Test Suite =http://lnxtestrail.ibi.com/testrail/index.php?/suites/view/10032&group_by=cases:section_id&group_order=asc
Test Case = http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/2227743
TestCase Name = Report-Other: Verify by user can show specific # of records
'''
import unittest, time
from common.lib.basetestcase import BaseTestCase
from common.pages import active_miscelaneous, ia_run
from common.wftools import active_report
from common.lib import utillity

class C2511613_TestClass(BaseTestCase):

    def test_C2511613(self):
        
        """
            TESTCASE VARIABLES
        """
        Test_Case_ID='C2511613'
        miscelanousobj = active_miscelaneous.Active_Miscelaneous(self.driver)
        iarun = ia_run.IA_Run(self.driver)
        active_reportobj=active_report.Active_Report(self.driver)
        utillobj = utillity.UtillityMethods(self.driver)
        fex_name ="AHTML_ON_001a.fex"
        report_dataset_name="AHTML_ON_001a"
            
        """
            Step 01 : Sign in to WebFOCUS as a Basic user http://machine:port/{alias}
            Step 02 :Expand folder P292_S10032_G157266
                Execute the following URL:http://machine:port/{alias}/run.bip?BIP_REQUEST_TYPE=BIP_RUN&BIP_folder=IBFS%253A%252FWFC%252FRepository%252FP292_S10032_G157266%252FAHTML_OFF&BIP_item=AHTML_OFF_001a.fex
        """
        active_reportobj.run_active_report_using_api(fex_name, column_css="#ITableData0 tr:nth-child(2) td:nth-child(2)", synchronize_visible_element_text="C141")
        
        """
             Step 03 : Verify the report is generated.
        """
        miscelanousobj.verify_page_summary('0','107of107records,Page1of2','Step 03.1 : Verify page summary')
        utillobj.verify_data_set('ITableData0', 'I0r', report_dataset_name+".xlsx", "Step 03.2: verify 107 records of report data")
        
        """
             Step 04 : Click dropdown next to any column and select Show Records > 5 Records
                Verify report show only 1st five records from the top.
        """
        miscelanousobj.select_menu_items("ITableData0",2, "Show Records", "5 Records")
        time.sleep(4)
#         iarun.create_table_data_set('#ITableData0',Test_Case_ID+'_DataSet_Step01.xlsx')
        iarun.verify_table_data_set('#ITableData0',Test_Case_ID+'_DataSet_Step01.xlsx', 'Step 04.1 : Verify Product column is no longer displayed on the report.')
        miscelanousobj.verify_page_summary('0','107of107records,Page1of22','Step 04.2 : Verify page summary')
        
        """
             Step 05:Dismiss the window and logout.http://machine:port/ibi_apps/service/wf_security_logout.jsp
        """
if __name__ == '__main__':
    unittest.main()