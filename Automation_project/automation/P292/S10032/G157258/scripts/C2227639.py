...
from common.pages import visualization_metadata,visualization_resultarea, visualization_ribbon,visualization_properties,define_compute
...
class C2227639_TestClass(BaseTestCase):
    ...
    def test_C2227639(self):
        ...
        parent_css="#fldCreatorCancelBtn"
        utillobj.synchronize_with_number_of_element(parent_css, 1, 30)
        field_name = self.driver.find_element_by_id("fname")
        utillobj.set_text_field_using_actionchains(field_name, 'Category', keyboard_type=True)
        time.sleep(2)
        field_format = self.driver.find_element_by_id("fformat")
        utillobj.set_text_field_using_actionchains(field_format, 'A90V', keyboard_type=True)
        time.sleep(2)
        ...
        """
        Step 06: Verify Define field "Category" appears in the Data pane
        """
        metaobj.verify_data_pane_field('Customer',"Category",1,"Step 06: Verify define field Category appears in data pane")
        time.sleep(2)
         
        """
        Step 07: Double click "Category" to add field to Canvas
        """
        metaobj.datatree_field_click('Dimensions->Category', 2, 1)
        time.sleep(5)
        ...
        """
        metaobj.drag_drop_data_tree_items_to_filter('Dimensions->Category', 1)
        time.sleep(5)
        ...
        """
        parent_css="[class*='riser!s']"
        utillobj.synchronize_with_number_of_element(parent_css, 21, 30)
        ...
        Step17: Save as "C2227639" > Click Save
        ...
        http://machine:port/ibi_apps/ia?item=IBFS%3A%2FWFC%2FRepository%2FS10032%2FC2227639.fex&tool=idis
        """
