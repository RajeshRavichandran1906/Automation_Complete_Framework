...
class C2227649_TestClass(BaseTestCase):
    ...
    def test_C2227649(self):
        ...
        """
        utils.infoassist_api_login('idis','baseapp/wf_retail_lite','P292/S10032_visual_1', 'mrid', 'mrpass')
        ...
        visualization.wait_for_visible_text(query_tree_css, "Sale,Year")
        ...
        d=utils.get_attribute_value(elem,'text')
        utils.asequal(d['text'],'Range',"Step05.a: Verify Operator dialog")
        
        elem1=driver.find_element_by_css_selector("#avfFromValue input")
        d=utils.get_attribute_value(elem1, "int_value")
        print(d['int_value'])
        utils.asequal(d['int_value'],"2011","Step05.b: Verify range to value")
        
        elem1=driver.find_element_by_css_selector("#avfToValue input")
        d=utils.get_attribute_value(elem1, "int_value")
        print(d['int_value'])
        utils.asequal(d['int_value'],"2017","Step05.c: Verify range to value")   
        ...
        utils.synchronize_with_number_of_element(parent_css, 3, visualization.home_page_long_timesleep)
        parent_css= "#MAINTABLE_wbody1 svg g text[class*='yaxis-labels']"
        utils.synchronize_with_number_of_element(parent_css, 6, visualization.home_page_long_timesleep)
        time.sleep(5)
        ...
        utils.verify_chart_color("MAINTABLE_wbody1", "riser!s0!g0!mbar", "bar_blue", "Step 08.c: Verify first bar color")
        time.sleep(5)
        ...
        utils.verify_chart_color("MAINTABLE_wbody1", "riser!s0!g0!mbar", "bar_blue", "Step 10.c: Verify first bar color")
        time.sleep(5)
        #bar=['Sale Year:2016', 'Cost of Goods:$325,821,316.00', 'Filter Chart', 'Exclude from Chart', 'Drill down to Sale Quarter']
        ...
        """
        time.sleep(5)
        slider_obj = utils.validate_and_get_webdriver_object("#sliderPROMPT_1 div[id^='slider_'] [class^='ui-slider-handle']", "Slider css")
        core_utils.python_left_click(slider_obj)
        pyautogui.hotkey('ctrl', 'up')

#         propertyobj.move_slider_measure('#sliderPROMPT_1','min', 'right', 1,'int') #2172012.85 This function has issue since done script level  
        chart_obj = utils.validate_and_get_webdriver_object("#PROMPT_1_cs", "Chart css")
        core_utils.python_left_click(chart_obj)
        ...
        time.sleep(10)
        parent_css="#MAINTABLE_wbody1 svg g text[class*='mgroupLabel']"
        utils.synchronize_with_number_of_element(parent_css, 2, visualization.home_page_long_timesleep)
        parent_css= "#MAINTABLE_wbody1 svg g text[class*='yaxis-labels']"
        utils.synchronize_with_number_of_element(parent_css, 6, visualization.home_page_long_timesleep)
        ...
        utils.verify_chart_color("MAINTABLE_wbody1", "riser!s0!g0!mbar", "bar_blue", "Step 12.c: Verify first bar color")
        time.sleep(10)
        #bar=['Sale Year:2016', 'Cost of Goods:$325,821,316.00', 'Filter Chart', 'Exclude from Chart', 'Drill down to Sale Quarter']
        ...
        visualization.wait_for_visible_text("#MAINTABLE_wbody1", "Sale")
            
        """
        Step 14: Click Save in the toolbar
        Step 15: Save as "C2227649" > Click Save
        """
        ribbonobj.select_top_toolbar_item('toolbar_save')
        visualization.wait_for_visible_text('#IbfsOpenFileDialog7_btnCancel', "Cancel")
        utils.ibfs_save_as(Test_Case_ID)
        utils.synchronize_until_element_disappear("#IbfsOpenFileDialog7_btnCancel", visualization.home_page_short_timesleep)
            
        """
        Step 16: Logout: http://machine:port/ibi_apps/service/wf_security_logout.jsp
        """
        utils.infoassist_api_logout()
        time.sleep(5)
            
        """
        Step 17: Reopen fex using IA API: http://machine:port/ibi_apps/ia?item=IBFS%3A%2FWFC%2FRepository%2FS10032%2FC2227649.fex&tool=idis
        """
        utils.infoassist_api_edit(Test_Case_ID, 'idis', 'S10032_visual_1',mrid='mrid',mrpass='mrpass')
        ...
        utils.verify_chart_color("MAINTABLE_wbody1", "riser!s0!g0!mbar", "bar_blue", "Step 18.c: Verify first bar color")
        time.sleep(15)
        #bar=['Sale Year:2016', 'Cost of Goods:$325,821,316.00', 'Filter Chart', 'Exclude from Chart', 'Drill down to Sale Quarter']
