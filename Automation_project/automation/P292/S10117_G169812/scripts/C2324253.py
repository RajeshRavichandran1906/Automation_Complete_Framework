...
class C2324253_TestClass(BaseTestCase):
    ...
    def test_C2324253(self):
        ...
        '''
        mainpage.verify_repositery_item(folder_path,'Tree Functionality Testing', msg="3.1")
        ...
        resultobj.wait_for_property(parent_css, 1, expire_time=50, string_value="Domains", with_regular_exprestion=True)
        time.sleep(1)
        mainpage.select_repository_menu(folder_path+'->BIP_V4_Portal->Test_images','Cut')
        ...
        mainpage.verify_repositery_item(folder_path+'->Tree Functionality Testing','Test_images',item_exit=True, msg="8")
        
        '''
        Step 08.1 : Verify that the Test images document is no longer in its main folder 
        '''
        self.driver.refresh()
        parent_css = "#PortalResourcevBOX table tr td"
        resultobj.wait_for_property(parent_css, 1, expire_time=50, string_value="Domains", with_regular_exprestion=True)
        time.sleep(1)
        mainpage.verify_repositery_item(folder_path+'->BIP_V4_Portal','Test_images',item_exit=False, msg="8.1")
