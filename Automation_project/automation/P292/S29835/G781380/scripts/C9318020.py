...
@author: varun
Testcase Name : Adding optional static radio button control to the Page
Testcase ID : http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/9318020
'''
import unittest
...
class C9318020_TestClass(BaseTestCase):
    
    def test_C9318020(self):
        """
        Test case objects
        """
        ...
        EXPECTED_DEFAULT_FILTER_VALUE=['All']
        EXPECTED_CONTAINER_BUTTONS=['Maximize', 'Options']
        EXPECTED_PAGE_HEADER_BUTTONS=['Refresh', 'Show filters']
        EXPECTED_CONTAINERS=['20 - Single Select Static Optional']
        EXPECTED_FILTER_CONVERTER_CONTROLS=['Radio', 'Button set']
        EXPECTED_FILTER_RADIOGROUP_OPTIONS=['All', 'South America', 'EMEA', 'North America']
        ...
        """
        Step 5: Drag "20 - Single Select Static Optional" report into the page canvas from P292_S10660 > G433312 > Reference Items folder.
        ...
        pd_design.wait_for_visible_text("table[summary]>tbody>tr:nth-child(2)>td", visble_element_text='EMEA', time_out=main_page_obj.home_page_long_timesleep)
        pd_design.verify_html_report_data_set(DATA_SET_NAME1, 'Step 05.07 : Verify selected condition applied in the page')
        ...
        pd_design.verify_filter_dropdown_is_optional(FILTER_CONTRTOL_NAME, 'Step 06.03 : Verify {0} filter drop down control is optional'.format(FILTER_CONTRTOL_NAME))
        pd_design.verify_selected_value_of_filter_dropdown(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_FILTER_VALUE, 'Step 06.04 : Verify {0} option selected as default in filter drop down'.format(EXPECTED_DEFAULT_FILTER_VALUE))
        ...
        Verify dropdown control converted to radio button control and "All" radio button is selected.
        """
        ...
        pd_design.verify_filter_radio_group_is_optional(FILTER_CONTRTOL_NAME, 'Step 08.03 : Verify {0} filter radio group control is optional'.format(FILTER_CONTRTOL_NAME))
        ...
        pd_design.verify_selected_options_in_filter_radio_group(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_FILTER_VALUE, 'Step 08.05 : Verify {0} option selected as default in filter radio group control'.format(EXPECTED_DEFAULT_FILTER_VALUE))
        ...
        Verify page loads successfully in preview, scroll down the report and verify all values are available in the report.
        """
        ...
        pd_preview.verify_filter_radio_group_is_optional(FILTER_CONTRTOL_NAME, 'Step 10.06 : Verify {0} filter radio group control is optional'.format(FILTER_CONTRTOL_NAME))
        ...
        pd_preview.verify_selected_options_in_filter_radio_group(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_FILTER_VALUE, 'Step 10.08 : Verify {0} option selected as default in filter radio group control'.format(EXPECTED_DEFAULT_FILTER_VALUE))
        ...
        pd_preview.verify_html_report_has_vertical_scrollbar('Step 10.12 : Verify container report has vertical scroll bar in preview')
        pd_preview.scroll_down_html_report()
        pd_preview.verify_html_report_data_set(DATA_SET_NAME1, 'Step 10.13 : Verify all values are available')
        ...
        pd_preview.switch_to_container_frame(CONTAINER_ITEM)
        pd_preview.wait_for_number_of_element("table[summary]>tbody>tr", 9, time_out=main_page_obj.home_page_short_timesleep)
        pd_preview.verify_html_report_data_set(DATA_SET_NAME2, 'Step 11.03 : Verify selected condition applied in the page')
        pd_preview.switch_to_default_page()
        ...
        """
        Step 15: Right click created page and select Run in New Window.
        Verify page runs successfully with "All" filter condition.
        """
        pd_design.run_page_designer(TEST_CASE_ID)
        core_util_obj.switch_to_frame(".output-area .ibx-iframe-frame")
        ...
        pd_run.verify_filter_radio_group_is_optional(FILTER_CONTRTOL_NAME, 'Step 15.04 : Verify {0} filter radio group control is optional'.format(FILTER_CONTRTOL_NAME))
        ...
        pd_run.verify_selected_options_in_filter_radio_group(FILTER_CONTRTOL_NAME, EXPECTED_DEFAULT_FILTER_VALUE, 'Step 15.06 : Verify {0} option selected as default in filter radio group control'.format(EXPECTED_DEFAULT_FILTER_VALUE))
        ...
        pd_run.wait_for_visible_text("table[summary]>tbody>tr:nth-child(2)>td", visble_element_text='EMEA', time_out=main_page_obj.home_page_long_timesleep)
        pd_run.verify_html_report_data_set(DATA_SET_NAME1, 'Step 15.10 : Verify page runs successfully with "All" filter condition.')
