...
Test Case = http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/2227674
Test case Name =  Filter Aggregation - Percent
'''
...
class C2227674_TestClass(BaseTestCase):

    def test_C2227674(self):
        """
        TESTCASE VARIABLES
        """
        Test_Case_ID = 'C2227674'
        ...
        """
        Step05: Click "Aggregation" dropdown menu > Select "Percent"
        """      
        time.sleep(6)
        ...
        utillobj.select_any_combobox_item(elem,'Percent',verify=True,expected_combobox_list=combo, msg="Step05: Verify list of aggregation functions")
        ...
        utillobj.asequal(d['text'],'Percent',"Step06.a: Verify Aggregation in Filter dialog")
             
        elem1=driver.find_element_by_css_selector("#avfFromValue input")
        d=utillobj.get_attribute_value(elem1, "float_value")
        utillobj.asequal(d['float_value'],"0.04","Step06.b: Verify From in Filter dialog")
             
        elem1=driver.find_element_by_css_selector("#avfToValue input")
        d=utillobj.get_attribute_value(elem1, "float_value")
        utillobj.asequal(d['float_value'],"23.79","Step06.c: Verify To in Filter dialog")
        ...
        """
        time.sleep(5)
        metaobj.verify_filter_pane_field('PCT (Cost of Goods)',1,"Step08:")
        ...
        propertyobj.verify_slider_range_filter_prompts('#ar_Prompt_1','min',0.04,'float',msg="Step09: Verify filter prompt range min values")
        propertyobj.verify_slider_range_filter_prompts('#ar_Prompt_1','max',23.79,'float',msg="Step09: Verify filter prompt range max values")
        ...
        time.sleep(5)
        bar=['Product Subcategory:Handheld', 'Cost of Goods:$20,576,916.00', 'Filter Chart', 'Exclude from Chart', 'Drill up to Product Category', 'Drill down to Model']
        resultobj.verify_default_tooltip_values("MAINTABLE_wbody1", "riser!s0!g7!mbar", bar, "Step 09: Verify bar value")
        ...
        Step12: Hover over riser for value "Handheld"
        ...
        if browser != 'Firefox':
            ...
            time.sleep(10)
        propertyobj.verify_slider_range_filter_prompts('#LOBJ1_cs','min',0.04,'float',msg="Step11: Verify filter prompt range min values")
        propertyobj.verify_slider_range_filter_prompts('#LOBJ1_cs','max',23.79,'float',msg="Step11: Verify filter prompt range max values")
        time.sleep(3) 
        parent_css="#MAINTABLE_wbody1 svg g text[class*='mgroupLabel']"
        resultobj.wait_for_property(parent_css, 21)
        parent_css= "#MAINTABLE_wbody1 svg g text[class*='yaxis-labels']"
        ...
        time.sleep(5)
        bar=['Product Subcategory:Handheld', 'Cost of Goods:$20,576,916.00', 'Filter Chart', 'Exclude from Chart', 'Drill up to Product Category', 'Drill down to Model']
        resultobj.verify_default_tooltip_values("MAINTABLE_wbody1", "riser!s0!g7!mbar", bar, "Step 13: Verify bar value")
        time.sleep(20)
        ele=driver.find_element_by_css_selector("div[id*='ibi$container$inner']")
        utillobj.take_screenshot(ele,'C2227674_Actual_step11', image_type='actual',x=1, y=1, w=-1, h=-1) 
        ...
        """
        Step16: Click Save as "C2158188" > Click Save
        """
        time.sleep(2)
        ...
        resultobj.wait_for_property(parent_css, 6)
        time.sleep(3)
        metaobj.verify_filter_pane_field('PCT (Cost of Goods)',1,"Step19:")
        propertyobj.verify_slider_range_filter_prompts('#ar_Prompt_1','min',0.04,'float',msg="Step19: Verify filter prompt range min values")
        propertyobj.verify_slider_range_filter_prompts('#ar_Prompt_1','max',23.79,'float',msg="Step19: Verify filter prompt range max values")
        time.sleep(5)  
        xaxis_value="Product Subcategory"
        yaxis_value="Cost of Goods"
        ...
        time.sleep(5)
        bar=['Product Subcategory:Handheld', 'Cost of Goods:$20,576,916.00', 'Filter Chart', 'Exclude from Chart', 'Drill up to Product Category', 'Drill down to Model']
        resultobj.verify_default_tooltip_values("MAINTABLE_wbody1", "riser!s0!g7!mbar", bar, "Step 19: Verify bar value")
        time.sleep(5) 
