...
class C2227699_TestClass(BaseTestCase):
    def test_C2227699(self):
        ...
        utillobj.infoassist_api_login('idis','new_retail/dimensions/wf_retail_product','P292/S10032_visual_3', 'mrid', 'mrpass')
        ...
        ia_ribbonobj.create_join("new_retail->dimensions->wf_retail_vendor", new_join=False)
         
        """    5. Drag and drop field "ID_VENDOR" from wf_retail_product to "ID_VENDOR" in wf_retail_vendor to create Join link    """
        ia_ribbonobj.create_join_link(0, "ID_VENDOR", 1, "ID_VENDOR")
        resultobj.wait_for_property("#dlgJoin line[marker-start]", 1)
        ...
        calculate_obj.enter_define_compute_parameter("Vendor", "A30V", "Vendor Name", 1)
        time.sleep(1)
        ...
        resultobj.wait_for_property(parent_css, 30)
        ...
        resultobj.wait_for_property(parent_css, 30)
        parent_css="#MAINTABLE_wbody1 rect[class*='riser']"
        resultobj.wait_for_property(parent_css, 60)
