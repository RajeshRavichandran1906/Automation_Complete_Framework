...
class C2228157_TestClass(BaseTestCase):
    ...
    def test_C2228157(self):
        ...
        Test_Case_ID1 = "IA-VAL-CHART-008"
        driver = self.driver
        ...
        """
        elem="#TableChart_1 [class='yaxis-majorGrid']"
        ...
        """ 
            ...
            Step 24:Enter Title = "IA-VAL-CHART-008".
            Step 25:Click "Save".
        """
        ...
        """  
            Step 27:Locate saved fex > Right mouse click > "Run".
        """
        utillobj.active_run_fex_api_login(Test_Case_ID1+".fex", "S10032_chart_1", 'mrid', 'mrpass')
        parent_css="#jschart_HOLD_0 [class='xaxisOrdinal-majorGrid']"
        resultobj.wait_for_property(parent_css,1)
        """  
            Step 28:Verify the same chart is displayed.
        """
        expected_font_color=utillobj.color_picker("red", 'rgba')
        actual_font_color=Color.from_string(driver.find_element_by_css_selector("#jschart_HOLD_0 [class='xaxisOrdinal-majorGrid']").get_attribute("stroke")).rgba
        ...
        time.sleep(1)
        expected_tooltip_list=['LAST_NAME:BANNING', 'CURR_SAL:$29,700.00']
        resultobj.verify_default_tooltip_values("jschart_HOLD_0", "riser!s0!g0!mbar!", expected_tooltip_list, "Step 28:07:Verify tooltip value")
        
        """ 
            Step 29:Close the chart run window.
        """
        utillobj.infoassist_api_logout()
        """  
            Step 30:Locate the saved fex > Right mouse click > "Edit".
        """
        utillobj.infoassist_api_edit(Test_Case_ID1, 'chart', 'S10660_chart_1', mrid='mrid', mrpass='mrpass')
        parent_css="#queryTreeWindow table tr:nth-child(3) td"
        resultobj.wait_for_property(parent_css, 1)
        """  
            Step  31:Verify IA is launched and the grid lines changes are preserved "in Live Preview".
        """
        expected_font_color=utillobj.color_picker("red", 'rgba')
        actual_font_color=Color.from_string(driver.find_element_by_css_selector("#TableChart_1 [class='xaxisOrdinal-majorGrid']").get_attribute("stroke")).rgba
