...
$relid = lc( $relid );
$targetFolder= (($OSNAME eq 'MSWin32') ? "$workspace\\qa\\automation\\$projectFolder\\$suiteFolder\\$subsectionFolder" : "$workspace/qa/automation/$projectFolder/$suiteFolder/$subsectionFolder");
my $Conf = {};
...
my @resp = (($OSNAME eq 'MSWin32') ? qx(\\\\bigrepos01\\bigscm\\admin\\bin\\getConf.pl $prodid $confid $relid) : qx(perl //bigrepos01/getConf.pl $prodid $confid $relid));
