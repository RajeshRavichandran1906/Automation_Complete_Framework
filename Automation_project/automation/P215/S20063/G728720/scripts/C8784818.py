...
Testcase ID : http://lnxtestrail.ibi.com/testrail/index.php?/cases/view/8784818
'''
...
class C8784818_TestClass(BaseTestCase):
    
    def test_C8784818(self):
        ...
        def verify_text(expected_text_list_1):
            ...
            for i in expected_text_list_1:
                ...
                if status == False:
                    break
            utillobj.asequal(True, status, "Step14.5: Verify text present in DeferredReportStatus window")
            
        "-------------------------------------------------------------------Test Case Variables--------------------------------------------------------------------------"
        TestCase_ID = "C8784818"
        USER_NAME='mrdevid'
        PASSWORD= 'mrdevpass'
        ...
        expected_y_axis_labels=['0', '30M', '60M', '90M', '120M', '150M']
        ...
        """
        time.sleep(sleep_time)
        calendar_icon_css="#calendar1_but"
        Date_picker_css="div.ui-datepicker"
        from_calendar_btn_obj=utillobj.validate_and_get_webdriver_object(calendar_icon_css, 'From date calendar button')
        from_calendar_btn_obj.click()
        ...
        """
        time.sleep(sleep_time)
        calendar_icon_css="#calendar2_but"
        Date_picker_css="div.ui-datepicker"
        to_calendar_btn_obj=utillobj.validate_and_get_webdriver_object(calendar_icon_css, 'To date calendar button')
        to_calendar_btn_obj.click()
        chart_obj.wait_for_number_of_element(Date_picker_css, 1, medium_wait)
        ...
        """
        Step 14: Click Refresh icon
        """
        refresh_icon_css="td a[href] img[src*='images'][title='Refresh']"
        refresh_icon=utillobj.validate_and_get_webdriver_object(refresh_icon_css, 'Refresh icon')
        refresh_icon.click()
        time.sleep(sleep_time)
        ...
        wnd_title=driver.title
        utillobj.asin(wnd_title,'Deferred Report Status','Step 14.1: Verify the Deferred Report Status window is displayed')
        ...
        utillobj.verify_object_visible(refresh_image_css, True, "Step14.2: Verify Refresh image object is displaying")
        utillobj.verify_object_visible(sort_image_css, True, "Step14.3: Verify sort image object is displaying")
        utillobj.verify_object_visible(delete_image_css, True, "Step14.4: Verify delete image object is displaying")
        ...
        for expected_text, actual_text in zip(expected_text_list_2, actual_text_list_2):
            if expected_text == actual_text:
                ...
                status=False
                break
        utillobj.asequal(status, True, "Step14.6: Verify text Refresh, Sort By, Delete, Help")
        
        href = self.driver.find_elements_by_css_selector("td a[href]")
        actual_href_count = len(href)
        utillobj.as_GE(actual_href_count, 8, "Step14.7: Verify hyperlink count in Deferred Status Report page")       
        ...
        utillobj.asequal("Date", actual_css, "Step14.8: Verify Date selected in Sort By dropdown")
        ...
        chart_obj.switch_to_new_window()
        chart_obj.wait_for_number_of_element(RISER_CSS, 42, medium_wait)
        chart_obj.verify_x_axis_label_in_run_window(expected_x_axis_labels, msg="Step 15:01: Verify x_axis label in runtime")
        chart_obj.verify_y_axis_label_in_run_window(expected_y_axis_labels, msg="Step 15:02: Verify y-axis label in runtime")
        chart_obj.verify_x_axis_title_in_run_window(expected_x_axis_title_list, msg="Step 15:03: Verify x_axis title at runtime")
        chart_obj.verify_y_axis_title_in_run_window(expected_y_axis_title_list, msg="Step 15:04: Verify y_axis title at runtime")
        chart_obj.verify_number_of_risers(parent_css_with_tag_name, 7, 6, msg="Step 15:05: Verify number of risers at runtime")
        chart_obj.verify_chart_color(chart_parent_run_css, 'riser!s0!g0!mbar!', 'bar_blue', "Step 15:06: Verify chart color")
        chart_obj.verify_legends_in_run_window(expected_legends, msg="Step 15:07: Verify legends")
        
        """
        Step 16: Close the output and Deferred window
        """
        ...
        """
        Step 17: Click 'Schedule' button under Run Options
        """
        ...
        wnd_title=driver.title
        utillobj.asin(wnd_title,'Distribution Method Selection','Step 17.1: Verify the schedule window is dispalyed')
        ...
        utillobj.asequal(actual_rc_label, 'Please choose distribution method:','Step 17.2: Verify the ReportCasterDistribution label')
        ...
        utillobj.as_GE(actual_dist_count, 2, "Step 17.3: Verify Email and Report Library")
        
        """
        Step 18: Close the schedule window
        """
        chart_obj.switch_to_previous_window()
        if len(driver.window_handles) != 1:
            print("window not switched to main window")
