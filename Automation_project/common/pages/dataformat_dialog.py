...
class DataFormat_Dialog(BasePage) :
    ...
    data_format_dialog_css = '.pop-top'
    ...
    def invoke_dataformattesterdlgonly_jsp(self):
        ...
        required_url=setup_url.replace('home8206', '') + jsp_file_directory_name + '/' + jsp_file_name + '.jsp'
    ...
    def set_text_to_format_to_parse_input_box(self, input_value):
        ...
        '''
        DataFormat_Dialog.set_text(self, input_value, DataFormat_Dialog.format_to_parse_input_box_css, 'Format to parse(input)')
    ...
    def close_data_format_dialog_using_ok_cancel_button(self, button_name, synch=True):
        '''
    ...
    def verify_checkbox_disable_in_dataformat_popup(self, label_name, checkbox_state, step_num):
        ...
        '''
        if checkbox_state not in ['enable', 'disable']:
            raise LookupError('Please pass label_name from "enable", "disable".')
        checkbox_css=DataFormat_Dialog.data_format_dialog_css + " [class*='wfc-dataformat-label'][data-ibx-name='cb{0}']".format(str(DataFormat_Dialog.cb_ibx_label_name[label_name.lower().replace(' ', '_')]))
        msg = 'Step {0}: Verify whether the {1} check box is {2}.'.format(str(step_num), label_name, checkbox_state)
        checkbox_label_elem = utillobject.validate_and_get_webdriver_object(self, checkbox_css, label_name + ' label')
        checkbox_label_class_attributes = utillobject.get_element_attribute(self, checkbox_label_elem, 'class')
        if checkbox_state == 'enable':
            utillobject.as_notin(self, 'ibx-widget-disabled', checkbox_label_class_attributes, msg)
        else:
            utillobject.asin(self, 'ibx-widget-disabled', checkbox_label_class_attributes, msg)
    ...
    def set_text(self, textval, text_css, section_name, type_speed=1):
        '''
        Description: This will set the text.
        '''
        txtboxobj = utillobject.validate_and_get_webdriver_object(self, text_css, section_name+' Text Box')
        txtboxval = utillobject.get_element_attribute(self, txtboxobj, 'value')
        coreutilobj.python_left_click(self, txtboxobj)
        time.sleep(1)
        keyboard.send('end')
        time.sleep(1)
        if sys.platform == 'linux':
            ...
            time.sleep(1)
            for _ in range(len(txtboxval)+3):
                pykeyboard.tap_key(pykeyboard.backspace_key)#press backspace
                time.sleep(0.5)
            pykeyboard.type_string(str(textval), interval=int(type_speed))
        else:
            for i in range(len(txtboxval)+3):
                keyboard.send('\x08')
                time.sleep(0.5)
            del i
            keyboard.write(str(textval), delay=int(type_speed))
        utillobject.synchronize_with_visble_text(self, text_css, textval, 90, text_option='text_value')
    ...
    def modify_length_value_using_keybord_input(self, input_value):
        ...
        '''
        DataFormat_Dialog.set_text(self, input_value, DataFormat_Dialog.alpha_length_input_textbox_css, 'Length')
        
    def select_max_or_min_value_in_alpha_max_length_dropdown(self, value_type):
        '''
        This function is used to select the max or min value in the spinner box. This will continue until value is reached maximum or minimum limit.
        value_type='max' or 'min'
        '''
        arrow_css = '.ibx-spinner-btn-up' if value_type == 'max' else '.ibx-spinner-btn-down'
    ...
    def select_value_in_alpha_max_length_dropdown(self, value):
        '''
        This function is used to select max or min value based on user value. This will continue until value is equivalent with given value.
        value=2
        '''
        current_value = int(DataFormat_Dialog.get_max_length(self))
        arrow_css = '.ibx-spinner-btn-up' if current_value < value else '.ibx-spinner-btn-down' 
        spinnerbox_elem=utillobject.validate_and_get_webdriver_object(self, DataFormat_Dialog.alpha_dataformat_box_css, 'Spinnerbox')
        spinnerbox_arrow_ele = utillobject.validate_and_get_webdriver_object(self, arrow_css, 'Spinnerbox arrow css', spinnerbox_elem)
        while True :
    ...
    def select_date_format(self, format_name):
        '''
        Year will changed with current year. So, this function will replace with current year dynamically.
        Note: like it will recived format_name='2018/12/31' then it will convert to format_name='2019/12/31'(only year will convert with the current year.)
        '''
        date_format_dropdown_arrow_css = DataFormat_Dialog.date_format_parent_css + " [data-ibx-name='dateFormat'] .ibx-select-open-btn"
        try:
            expected_format_name = format_name.replace(re.findall('\d\d\d\d',format_name)[0], time.strftime("%Y", time.localtime()))
        except IndexError:
            expected_format_name = format_name
        try:
        ...
        for current_item in self.driver.find_elements_by_css_selector(DataFormat_Dialog.dropdown_popup_items_css):
            if current_item.text.strip() == expected_format_name:
                get_item=True
                coreutilobj.left_click(self, current_item)
                break
        if get_item == None:
            raise ValueError('The requested item [' + expected_format_name + '] is not available inside date format dropdown popup.')
    ...
    def verify_date_format(self, format_name, step_no):
        ...
        msg = 'Step {0}: Verify Date format value displays {1}.'.format(str(step_no), expected_format_name)
        utillobject.asequal(self, expected_format_name, date_format_value, msg)
    ...
    def modify_max_length_value_using_keybord_input(self, input_value):
        DataFormat_Dialog.set_text(self, input_value, DataFormat_Dialog.numeric_length_input_textbox_css, 'Max Length')
    ...
    def modify_decimal_place_value_using_keybord_input(self, input_value):
        DataFormat_Dialog.set_text(self, input_value, DataFormat_Dialog.numeric_decimal_input_textbox_css, 'Max Length')
    ...
    def verify_symbol_position_disabled(self, step_number, checkbox_state='enable'):
        ...
        if checkbox_state == 'enable':
            utillobject.as_notin(self, 'ibx-widget-disabled', symbol_position_label_class_attributes, label_msg)
        else:
        ...
        if checkbox_state == 'enable':
            utillobject.as_notin(self, 'ibx-widget-disabled', symbol_position_group_class_attributes, group_button_msg)
        else:
    ...
    def modify_format_using_keybord_input(self, input_value):
        ...
        '''
        DataFormat_Dialog.set_text(self, input_value, DataFormat_Dialog.custom_format_input_css, 'Format')
