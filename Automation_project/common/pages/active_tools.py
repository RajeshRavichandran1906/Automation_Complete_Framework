...
class Active_Tools(BasePage):
    ...
    def pivot_tool_drag_drop_items(self, table_id, from_column, item_name, item_position, to_column, to_row_num, **kwargs):
        ...
        draggable_xy_dict=core_utilobj.get_web_element_coordinate(self, draggable)
        droppable_xy_dict=core_utilobj.get_web_element_coordinate(self, droppable)
        core_utilobj.move_to_element(self, draggable)
        core_utilobj.drag_and_drop_without_using_click(self, draggable_xy_dict['x'], draggable_xy_dict['y'], droppable_xy_dict['x'], droppable_xy_dict['y'])
        
#         if Global_variables.browser_name in ['firefox', 'ie']:
#             utillity.UtillityMethods.drag_to_using_pyautogui(self, draggable, droppable, **kwargs)
#         else:
...
#         time.sleep(2)
    ...
    def grid_tool_select_aggregate_function(self, table_id, item_name, item_position, function_name, **kwargs):
        ...
        utillity.UtillityMethods.default_left_click(self,object_locator=func_name,**kwargs)
        time.sleep(2)
...
#         time.sleep(2)
    ...
    def grid_tool_verify_columns(self, table_id, colnum, expected_list, msg):
        ...
        if Global_variables.browser_name == 'edge' :
            actual_list = [column.strip() for column in actual_list if column.strip() != '']
        utillity.UtillityMethods.asequal(self, expected_list, actual_list, msg)
...
#             trow_id = 'pta_0_'
        ...
        if to_row_num == 0:
            ...
            destination_elem = "#" + table_id + " tbody > tr:nth-child(1) > td:nth-child(" + str(tcol) + ") td[id^='" + trow_id +"']" + "[id$='" + str(to_row_num) + "_2']"
            print(destination_elem)
            droppable = self.driver.find_element_by_css_selector(destination_elem)
...
#         time.sleep(2)
    ...
    def drag_drop_grid_tool_column(self,table_id, source_column_name, target_column_name, **kwargs):
        '''
        Description : This function used to drag and drop grid tool columns to change order 
        :param : table_id ='gridtoolt1'
        :param : source_column_name ='Category'
        :param : source_column_name ='Product'
        :usage : drag_drop_grid_tool_column('gridtoolt1', 'Category', 'Product')
        '''
        columns_css="#"+table_id+"> tbody > tr:nth-child(1)> td:nth-child(1) >table>tbody>tr"
        grid_columns=self.driver.find_elements_by_css_selector(columns_css)
        columns_name=[col.text.strip().replace('\n',' ') for col in grid_columns]
        source_element=grid_columns[columns_name.index(source_column_name)]
        target_element=grid_columns[columns_name.index(target_column_name)+1]
        
        source_xy_dict=core_utilobj.get_web_element_coordinate(self, source_element)
        target_xy_dict=core_utilobj.get_web_element_coordinate(self, target_element)
        core_utilobj.move_to_element(self, source_element)
        core_utilobj.drag_and_drop_without_using_click(self, source_xy_dict['x'], source_xy_dict['y'], target_xy_dict['x'], target_xy_dict['y'])
        #if self.browser == 'Firefox':
#         if Global_variables.browser_name in ['firefox', 'ie']:
#             utillity.UtillityMethods.drag_to_using_pyautogui(self, source_element, target_element, **kwargs)
#         else:
#             dragdrop = ActionChains(self.driver).drag_and_drop(source_element, target_element)
#             dragdrop.perform()
#         time.sleep(3)