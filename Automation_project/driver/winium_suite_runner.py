...
print("build_credit ", build_credit)
...
appstudio_folder_name = 'appstudio'
...
appstudio_dir = os.path.join(tool_root_dir, appstudio_folder_name)
...
print('appstudio directory [' + appstudio_dir + ']')
...
special_runner = RunnerFactory.RunnerFactory.getInstance(product, asconf, release)
...
old_suite_appstudio = os.path.join(subsection_dir, appstudio_folder_name)
if os.path.isdir(old_suite_common):
    os.system('rm -r ' + old_suite_common)
shutil.move(common_dir, subsection_dir)
if os.path.isdir(old_suite_appstudio):
    os.system('rm -r ' + old_suite_appstudio)
shutil.move(appstudio_dir, subsection_dir)
...
#         print('test : ',test,' fail : ',fail, ' type : ',type(fail))
#         print('test : ',test,' err : ',err, ' type : ',type(err))
        #failures
        if fail != '0':
            status_to_testrail = failed_status
